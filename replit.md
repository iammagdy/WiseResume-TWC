...
- **Glass classes**: CSS compatibility aliases still defined in index.css; Phase 4 replaced ALL glass-* usage across every page and component with direct Tailwind classes (bg-card, bg-background/95, shadow-soft, etc.). Phase 5 cleaned up remaining bare `glass` class (no hyphen) from headers in ApplicationsPage, UploadPage, PreviewPage, EditorHeader, MobileLayout; removed all semi-transparent patterns (bg-card/50, bg-card/60, bg-card/80, border-border/20-50, backdrop-blur-xl/md) from InterviewPage, ResignationLetterNewPage, QR pages, SharePage, DevToolsPage; fixed duplicate border classes from Phase 4 sed in NotificationsPage, JobDetailPage, ApplicationTrackerPage headers. Zero glass references or semi-transparent surface/border patterns remain in TSX files. Portfolio style name "glass-pro" (a data value, not CSS class) and Badge variant="glass" (a component variant) are intentionally preserved.
