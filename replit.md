...
- **Glass classes**: CSS compatibility aliases still defined in index.css; Phase 4 replaced ALL glass-* usage across every page and component with direct Tailwind classes (bg-card, bg-background/95, shadow-soft, etc.). Zero glass-elevated/surface/card/input/header/border-glow references remain in TSX files. Portfolio style name "glass-pro" (a data value, not CSS class) is intentionally preserved.
