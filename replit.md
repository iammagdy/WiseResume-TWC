...
- **Admin DevKit**: A password-protected admin panel. **Mission Control** is the default landing tab — a single-screen status board (Deploy, AI Providers, Email, Database, Secrets, Recent Failures) that auto-refreshes every 60 s via `useVisibleInterval` and deep-links to detailed panels. The aggregator edge function `admin-mission-control` returns the full payload in one round trip and tracks secret rotation timestamps in `app_settings` (key: `secret_rotation_metadata`), surfacing any secret not rotated in 90+ days as "stale". Other tabs include Analytics, Live Activity, Deployment, Audit Log, AI Provider panels, and Users. The **User Detail Drawer** now has a lazy-loaded **Activity tab** showing account summary, content stats, and up to 50 usage events from `usage_events` filterable by event type.
