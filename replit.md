...
- **Editor AI smoke-test bypass**: The 5 active Editor AI edge functions (`editor-ai`, `resume-section-ai`, `tailor-resume`, `smart-fit-rewrite`, `agentic-chat`) check for the `x-smoke-test: true` request header and return a synthetic 200 response without calling AI or deducting credits. Implemented via `supabase/functions/_shared/smokeTest.ts`. DevKit smoke tests cover all 4 `editor-ai` sub-actions via `editor-ai-*` test entries. The 4 legacy standalone functions (`analyze-resume`, `recruiter-simulation`, `suggest-template`, `optimize-for-linkedin`) were retired in Task #41 (source stubs) and Task #44 (Supabase undeployment).
- **ai_routing_config**: Active rows: `editor-ai`, `resume-section-ai`, `tailor-resume`, `smart-fit-rewrite`, `agentic-chat` (+ platform-wide rows). The 4 legacy Editor AI rows (`analyze-resume`, `recruiter-simulation`, `suggest-template`, `optimize-for-linkedin`) were removed by migration `20260603000000_retire_legacy_editor_ai_routing_config.sql` (Task #41). Deployed function count: 70/100 (30 free slots) as of Task #44.
