...
- **Post-push QA correction:** Arabic Subscription browser QA exposed the literal `app.subscription` title in the shared workspace top bar. `AppWorkspaceTopBar.tsx` now maps Analytics and Subscription to `app.aiStudio.analyticsPage.title` and `app.aiStudio.subscriptionPage.title`; focused Arabic coverage guards both mappings. A new Preview deployment and browser rerun are required.
