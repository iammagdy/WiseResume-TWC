...
- **legacy payment provider subscription webhook has a clear runtime blocker.** `appwrite-hubs/legacy-payment-webhook/src/main.js` references `rawBody`, which is not defined in the handler.
...
| legacy payment provider webhook runtime failure | Critical | Handler uses `rawBody` without defining it, so subscription events may fail before processing. |
...
| Production Vercel/Appwrite/Resend/legacy payment provider/Sentry dashboards were not verified | High | Repo evidence cannot prove latest Vercel deployment, production env vars, function env vars, webhook logs, email delivery, or monitoring alerting. |

## Launch Recommendation

Do **not** open the product to real users yet.

Launch should wait until all P0 items in `prioritized-improvement-plan.md` are resolved and verified through a production smoke test using a real production test account, Vercel deployment logs, Appwrite function logs, Resend delivery logs, legacy payment provider webhook logs, and Sentry event visibility.
