...
- RevenueCat dashboard webhook settings were not read or changed live.
...
- The RevenueCat webhook ESM package metadata warning remains in local Node tests because its local package does not declare `"type": "module"`.
- Production verification still requires a real deployment or preview deployment plus Appwrite/Vercel/RevenueCat/Resend/Sentry console checks.
