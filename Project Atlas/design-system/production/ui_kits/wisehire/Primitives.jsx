...
function Button({ variant = 'primary', size = 'md', icon, iconRight, children, onClick, className = '', style = {} }) {
  ...
  const variants = {
    primary: { background: '#1D4ED8', color: '#fff', boxShadow: '0 4px 14px -2px rgb(29 78 216 / 0.35)' },
    outline: { background: 'var(--wh-card, #fff)', color: 'var(--wh-fg, #18181b)', border: '1px solid var(--wh-border, #e5e7eb)' },
    ghost: { background: 'transparent', color: 'var(--wh-muted-fg, #52525b)' },
    secondary: { background: '#dbeafe', color: '#1D4ED8' },
    icon: { background: 'transparent', color: 'var(--wh-muted-fg, #52525b)', width: 38, height: 38, padding: 0, borderRadius: 10 },
  };
  return (
...
function Card({ children, className = '', style = {}, hover = false }) {
  return (
    <div className={`wh-card ${hover ? 'wh-card-hover' : ''} ${className}`} style={style}>
      {children}
    </div>
  );
}
...
function Pill({ children, variant = 'primary', icon }) {
  const colors = {
    primary: { bg: 'rgb(29 78 216 / 0.08)', fg: '#1D4ED8', border: 'rgb(29 78 216 / 0.22)' },
    ...
    neutral: { bg: 'var(--wh-muted, #f3f4f6)', fg: 'var(--wh-fg, #374151)', border: 'var(--wh-border, #e5e7eb)' },
    purple:  { bg: 'rgb(139 92 246 / 0.1)', fg: '#7c3aed', border: 'rgb(139 92 246 / 0.22)' },
  };
...
function ScoreBar({ score, w = 60 }) {
  const color = score >= 85 ? '#16a34a' : score >= 75 ? '#1D4ED8' : score >= 65 ? '#60a5fa' : '#94a3b8';
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
      <div style={{ width: w, height: 5, borderRadius: 99, background: 'rgb(29 78 216 / 0.1)', overflow: 'hidden' }}>
        <div style={{ width: `${score}%`, height: '100%', borderRadius: 99, background: color, transition: 'width .5s ease' }} />
      </div>
      <span style={{ fontSize: 11.5, fontWeight: 700, color, minWidth: 22, textAlign: 'right' }}>{score}</span>
    </div>
  );
}

function Avatar({ initials, size = 32, idx = 0 }) {
  const bgs = [
...
Object.assign(window, { Icon, Button, Card, Pill, ScoreBar, Avatar });
