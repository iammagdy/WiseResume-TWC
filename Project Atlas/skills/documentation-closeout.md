...
2. **Evaluate What's New Eligibility (MANDATORY GATE)**: Every customer-impacting product, feature, or bug-fix task closeout MUST evaluate release eligibility according to [`Project Atlas/skills/whats-new-maintenance.md`](./whats-new-maintenance.md) and record an explicit decision:
   ...
   * **`WHATS_NEW_DEFER_UNTIL_PRODUCTION`**: User-facing change is implemented and merged, but production deployment and live browser QA are pending. Explicitly record what deployment or runtime evidence is still missing before publication.
   > Customer-impacting product, feature, or bug-fix tasks may NOT be reported fully closed without an explicit What's New decision recorded in the session handover. Internal, documentation-only, or administrative housekeeping tasks may record `WHATS_NEW_NOT_REQUIRED` or omit this evaluation when not applicable.
