...
**Status:** `PR301_FINAL_CODE_REVIEW_READY` (`IMPLEMENTED_UNVERIFIED` / `TESTED_LOCAL`, `PAYPAL_PRODUCTION_READY = NO`, Branch: `feat/paypal-refund-reversal-policy`, Target: `main`, PR #301) — Option B refund & reversal provider-contract hardening complete across backend, schema, coupons, frontend, and tests. Awaiting owner review of feature PR #301.
**Location:** `Project Atlas/WHERE_WE_STOPPED.md`

## Current Active Handover — PayPal Refund & Reversal Provider-Contract Hardening (Option B) (2026-09-07)

* **Workstream:** `PR301_FINAL_CODE_REVIEW_READY` (`IMPLEMENTED_UNVERIFIED` / `TESTED_LOCAL`, `PAYPAL_PRODUCTION_READY = NO`).
...
* **Scope & Implementation:**
  ...
  - **Partial Refund:** Preserves current entitlement and recurring renewal; recorded in ledger only (`partial_refund_recorded`). Relies strictly on PayPal's authoritative transaction status (`PARTIALLY_REFUNDED` preserves entitlement, `REFUNDED` triggers full refund policy) without local balance arithmetic.
  - **Historical Refund/Reversal:** When refunded/reversed `payment_id` is older than `last_entitlement_payment_id` and provider records / ledger confirm a newer payment supports entitlement, active state is untouched (`historical_refund_ignored` / `historical_reversal_ignored`). For reversals, authoritative payment identity and timestamp are queried from the ledger `PAYMENT.SALE.COMPLETED` event by `payment_id` (not reversal webhook arrival time); delayed older reversals arriving after newer payments are ignored with zero state mutation, and unresolved historical correlation fails closed (`unresolved_historical_reversal_correlation`).
  ...
  - **Additive Schema Definition:** `setup_paypal_schema.cjs` updated with additive attributes `last_entitlement_payment_id` (indexed with `last_payment_idx` ASC), `last_entitlement_payment_timestamp_ms`, `renewal_cancellation_pending` on `paypal_subscription_state`, and `payment_id` (indexed with `payment_idx` ASC) on `paypal_event_ledger`.
...
* **Test Verification Baseline:**
  ...
  - `node --test tests/hubs/paypal-webhook.test.cjs`: 130 / 130 passing (100%), including 49-case refund/reversal/tombstone/pagination/reversal-hardening matrix.
  - All hub test suites (`node --test tests/hubs/*.test.cjs`): 358 / 358 passing across 58 suites (100%).
  ...
  - DevKit source hashes: `src/lib/devkit/sourceHashes.generated.json` recomputed and verified (`paypal-webhook: 472d08bb811ed1038cb1cdec5043569a2317866a0d1e8279b51db0d37d26d746`).
...
* **Governance Deviation Record — OWNER_AUTHORIZATION_WORKFLOW_VIOLATION:**
  - **Incident:** `OWNER_AUTHORIZATION_WORKFLOW_VIOLATION`
  ...
  - **Rollback Decision:** **NO ROLLBACK PERFORMED**. No rollback is being performed because:
    ...
    2. First-party CI later passed across all checks.
    3. Production PayPal remains completely untouched and public checkout remains strictly fail-closed.
  - **Future Rule:** Any future code, merge, or deployment work requires explicit owner authorization again.
...
* **Customer UI Cancellation Verification:**
  ...
  - Designated Sandbox QA user logged in, inspected active Ultimate subscription with renewal date `October 6, 2026`.
...
* **Direct PayPal API Confirmation:**
  - Queried PayPal REST API for the verified Ultimate Sandbox subscription:
    * `status: "CANCELLED"`
    * `status_change_note: "User requested cancellation in subscription settings"`
    * `status_update_time: "2026-09-06T10:59:18Z"`
* **Authentic PayPal Webhook Processing & Delivery Distinction:**
  ...
  - The new `BILLING.SUBSCRIPTION.CANCELLED` event was observed through first automatic provider delivery without manual resend (created `2026-09-06T10:59:22.000Z`).
...
* **Authoritative Appwrite State & Resolver Contract:**
  - `coupons` `get-subscription` returns:
    ...
    ```
  - **Paid-Through Invariant:** `expires_at` is preserved as `2026-10-06T10:00:00.000Z` (NOT NULL!). Effective plan remains `premium`. Unlimited AI quota in `ai-gateway` remains fully accessible until expiration date; server-side AI entitlement follows unchanged resolver contract (no post-cancellation AI execution run).
...
* **Retained Gaps:**
  - `BILLING_CHECKOUT_DEVKIT_SOURCE_HASH_NOT_TRACKED_PRE_EXISTING`: Pre-existing gap retained; not claimed as fixed.
* **What's New Decision:**
  - `WHATS_NEW_DEFER_UNTIL_PRODUCTION`: The cancellation fix is customer-impacting, but public PayPal checkout remains disabled and integration is still Sandbox-only. Do not publish customer-facing What's New release notes yet.
* **Remaining Scope / Untested Scenarios (Production Blockers):**
  ...
  - Production PayPal remains strictly `UNTOUCHED / DISABLED` (`PAYPAL_PRODUCTION_READY = NO`).
  - Live webhook endpoint configuration: `UNVERIFIED_FOR_LIVE` (preserving approved custom domain `https://paypal-webhook.wiseresume.app` architecture; direct Appwrite execution endpoint is not the canonical public endpoint).
...
* **Node Hub Tests:** **150 / 150** passing across 4 suites:
  ...
  - `tests/hubs/paypal-webhook.test.cjs`: **66 / 66** (idempotency, POSTBACK verification, renewal state transitions).
...
* **Validation Evidence:**
  ...
  - `src/lib/__tests__/aiTailor-D1.test.ts` (7/7 pass).
  - `src/pages/__tests__/TailoringHubPage-recovery.test.tsx` (3/3 pass).
...
* **Work Accomplished & Merged to `main`:**
  1. **P1-1 Public Portfolio Contact Form & Routing Isolation:**
     ...
     - Updated `PortfolioContactForm` to invoke `send-portfolio-contact-email`. Action override guard in `src/lib/appwrite-functions.ts` spreads `action: 'send-portfolio-contact-email'` after `...finalPayload` to prevent caller payload override.
     ...
     - Added comprehensive unit test suites in `tests/hubs/email-service-portfolio-contact.test.cjs` (12/12 pass) and `src/lib/__tests__/contactRoutingRegression.test.ts` (7/7 pass).
...
* **Outbound Webhook Routing:**
  ...
  - Event Filters: Unchanged (7 supported events).
...
## Phase P2 Appwrite targeted deployment complete — 2026-08-30

* **Verdict:** `P2_TARGETED_DEPLOYMENT_VERIFIED_WITH_WARNINGS`. Workflow run `33310801069` (`target=billing-checkout,revenuecat-webhook`) completed successfully on `main` (`ba5a785ee54a84194fac8630af3370f9f3a9ccf0`).
* **Deployments:**
...
* **Schema Status:** `SCHEMA_RECONCILIATION_SUCCEEDED` — RevenueCat provider-state schemas and billing checkout schemas reported ready. No schema mutation was observed or reported.
...
## Phase P2 Appwrite source hash manifest recovery (PR #248 MERGED @ ba5a785e) — 2026-08-30

* **Verdict:** `SOURCE_HASH_RECOVERY_MERGED_TO_MAIN`. Source hash manifest regenerated (`revenuecat-webhook` hash updated to `704b896e0460187b21d84e1f42f208088baf4c3d128971f0c114137435e27cdd`) and merged via PR #248 (`ba5a785e`).
...
* **Historical boundary for run 33309686634:** 0 Appwrite Functions deployed in run `33309686634`. `BILLING_CHECKOUT_ENABLED=false` preserved.
...
* **Verdict:** `P1_PRODUCTION_BILLING_WIRING_MERGED_TO_MAIN`. All four targeted code changes implemented, committed, pushed, PR #247 opened and merged into `main` at merge SHA `78c0afc9c9bdc7c962d57f6fee1c8ad20e408526`. Tests pass 11/11 (revenuecat-webhook), 6/6 (billing-checkout-deployment), 7/7 (deployment-hardening), 4/4 (appwrite-function-policy). `git diff --check` clean.
...
* **Historical boundary for P1:** PR #247 merged; initial P2 workflow `33309686634` failed pre-deploy due to stale manifest; 0 Appwrite Functions deployed in P1; `BILLING_CHECKOUT_ENABLED=false`; `paymentsEnabled: false`; Production billing remained strictly disabled.
