...
**Status:** `FULL_POST_CHANGE_REGRESSION_AUDIT_PASS_WITH_WARNINGS` — PR #187, PR #188, PR #189, and PR #191 are merged into `main`; PR #191’s final main SHA is `b6cf2aa07ce94f160e048a8a02e86aaa0db7293b`; deployment status remains `NOT_DEPLOYED`; the owner-authorized targeted deployment task is pending final repository preflight and workflow dispatch; no Appwrite or manual Vercel deployment was performed
...
* **Deployment:** `NOT_DEPLOYED`. Workflow runs `31871663976` and `31875957559` failed at the security assertion (`permissionsIsArray=false, permissionCount=unknown, documentSecurity=false`). Run `31879539590` passed the `$permissions` gate and failed at the first attribute comparison: `request_id: required false (expected true)`. PR #189 corrected the Appwrite response-shape assertion; PR #191 then reconciled the complete live application-attribute contract as optional while preserving all types, indexes, defaults, and server-only security requirements. No function was deployed. No Appwrite Console, schema, permissions, secrets, environment variables, data, or manual Vercel deployment were changed.
...
* **Stop point:** PR #188, PR #189, and PR #191 are merged; local main is synchronized and clean. The complete live-contract repository fix is now on `main`; the final read-only repository preflight remains to be run before dispatching the exact four-target workflow. The current task authorizes that targeted deployment, but no deployment has yet occurred. Do not use direct setup scripts or Appwrite Console mutation. After deployment, reverify both public-share variants, the legacy DevKit panels, exact function/hash parity, and all required terminal states.

## AI runtime receipts schema contract remediation (2026-08-15; final live-contract reconciliation)

* **Status:** `MERGED_VALIDATED_NOT_DEPLOYED`. PR #187 established the earlier six-required-attribute hypothesis, but later live metadata and workflow evidence showed that hypothesis was not truthful. PR #191 (`fix/appwrite-runtime-receipts-final-preflight` → `main`) superseded it and merged with final main SHA `b6cf2aa07ce94f160e048a8a02e86aaa0db7293b`.
* **Failed workflows:** Runs `31871663976` and `31875957559` failed at the server-only security assertion. Run `31879539590` passed the `$permissions` gate and failed at `request_id: required false (expected true)`. No Appwrite function was deployed.
...
* **Deployment boundary:** `NOT_DEPLOYED`. No Appwrite setup or deployment, manual Vercel deployment, schema/permission mutation, secret/environment change, or production-data change occurred. The owner-authorized targeted deployment task may now proceed only through final repository preflight and the approved exact four-target workflow; no direct production setup script or Console mutation is permitted.
