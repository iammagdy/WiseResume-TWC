...
| AI | Appwrite `ai-gateway` Function | Routes 24+ features; provider pool: OpenRouter, Groq, DeepSeek |
...
- AI Hub — all 24+ features routed through `ai-gateway` Appwrite Function
- GitHub repo (`iammagdy/WiseResume-TWC`, `main`) — remote HEAD `2ffb68e`, file content matches Replit local, `git push` will work from a fresh clone
...
- Admin DevKit — throws `pending_appwrite_migration`
- Portfolio tracking / short-links — partially migrated (see Task #13 CHANGELOG entry); some features still on stubs
- Mobile app (`mobile/`) — still targets legacy backend, not migrated

### Active Task Queue (proposed/pending)
- **#26** — Post-deploy smoke test in `deploy-frontend.yml`
- **#27** — Wire `public/_headers` CSP into `.htaccess` so Apache enforces it
- **#29** — MERGED (git history reconciled)
- **#30** — Delete backup branch `sync-from-replit-2026-05-09` from GitHub
- Multiple data-layer and feature rebuild tasks (WiseHire, PDF, portfolio, etc.)
...
| `.github/workflows/deploy-appwrite-hubs.yml` | Deploy `appwrite-hubs/ai-gateway` to Appwrite Functions |
...
| `appwrite-hubs/ai-gateway/src/main.js` | Appwrite Function — AI router + coupon/billing logic |
| `CHANGELOG.md` | Technical change log (all tasks documented here) |

---
*Last updated: 2026-05-08 by session agent (Tasks #25, #28, #29)*
