...
**Targeted Deployment ID (`paypal-webhook`):** `6a9ea51d2ecf33ade11a` (Run `34118592362`)  
...
Following the genuine $10.00 USD refund of subscription sale `0B9419070U158972P` (Refund ID `1H603208GJ834104W`), PayPal emitted genuine webhook event `WH-39D23786BJ747394G-6NV67311UF312770M` (`PAYMENT.SALE.REFUNDED`). Because authentic PayPal refund event payloads omit `billing_agreement_id` and legacy pre-PR #299 records lacked payment identity, the webhook initially failed to correlate to the subscription and was marked `rejected` (`unresolved_subscription_correlation`). Furthermore, the event reclaim logic treated `rejected` events as terminal duplicates (`already_recorded`), blocking redelivery recovery.
...
After targeted deployment of `paypal-webhook` via GitHub Actions workflow run `34118592362`, a single genuine PayPal Sandbox redelivery was requested via `POST /v1/notifications/webhooks-events/WH-39D23786BJ747394G-6NV67311UF312770M/resend` (HTTP 202 Accepted). The webhook reclaimed the rejected ledger record, resolved subscription `I-58K84FGAFFHL` via Step 3 provider Sale fallback, executed Case C ("True Legacy Migration-on-Touch"), populated payment identity, and executed Option B full refund revocation:
- `plan`: `free`
- `effective_plan`: `free`
- `status`: `canceled`
- `expires_at`: `null`
...
1. **No Duplicate Refunds / No Additional Payments:**
   ...
   - No second refund was issued (the existing $10.00 USD refund for sale `0B9419070U158972P` was reused as authoritative evidence).
...
5. **Zero Synthetic / Simulated Signatures:**
   ...
   - Authentic PayPal Sandbox event `WH-39D23786BJ747394G-6NV67311UF312770M` was redelivered by PayPal's genuine notification broker.
...
| **Targeted Hub Deployment** | `deploy-appwrite-hubs.yml` (Run `34118592362`) | GitHub Actions workflow (`target=paypal-webhook`) | Deployment `6a9ea51d2ecf33ade11a` created with status `ready` in 1m41s | **PASS** |
...
| **Genuine Event Redelivery** | Event `WH-39D23786BJ747394G-6NV67311UF312770M` | `POST /v1/notifications/webhooks-events/.../resend` | HTTP 202 Accepted from PayPal Sandbox | **PASS** |
...
| **Step 3 Sale Fallback** | Sale `0B9419070U158972P` -> Sub `I-58K84FGAFFHL` | Provider REST query `GET /v1/payments/sale/{paymentId}` | Correlation succeeded; identity cross-checks verified matching QA user | **PASS** |
| **Case C Migration-on-Touch** | `I-58K84FGAFFHL` | Provider Transactions API query | Historical payment identity populated on state and ledger documents | **PASS** |
| **Option B Full Refund Revocation** | QA user `qa_pp_afbf725e` | `coupons` `get-subscription` function invocation | `plan: "free"`, `effective_plan: "free"`, `status: "canceled"`, `expires_at: null` | **PASS** |
...
1. `live_subscription_page_1.png`: Confirms the live `/subscription` page loaded under QA user `qa_pp_afbf725e`, displaying the `Free` plan hero, Free badge in the sidebar, and 0/5 AI credits.
