...
**Mode:** Auto-idempotency corrective implementation and Draft PR validation
...
The corrective review confirmed two additional defects. First, `findOptional()` converted every Appwrite read failure into `null`, allowing authoritative subscription/provider-state outages to look like Free. The correction returns sanitized `state_unavailable` before reservation or provider work. Second, omitted `idempotency_key` values shared a 24-hour fallback key, causing unrelated no-key plan attempts and retries to collide. The correction uses a unique server-generated attempt key when the client omits a key.
...
The future Function accepts only an authenticated request with `action=create-session`, an exact internal `plan` of `pro` or `premium`, and an optional bounded opaque `idempotency_key`. The chosen optional-key contract is Option B: omitted keys receive a unique server-generated per-attempt key; explicit keys retain deterministic replay/recovery semantics. The canonical Appwrite user is resolved from the Appwrite JWT server context. Browser-supplied `user_id`, `price_id`, `transaction_id`, environment, provider, return URL, and callback/status fields are rejected and are never used for authority.
...
The default policy is one active session per user and plan for 15 minutes, a 24-hour same-key replay window, and at most three creation attempts per user in ten minutes. Explicit same-key same-input requests recover the same checkout; explicit same-key different-input requests return `idempotency_conflict`. Omitted keys receive a unique server-generated attempt key, so no-key Pro and Premium requests do not collide and a failed no-key attempt does not stale-block a retry. The one-active user/plan lock still protects concurrent no-key requests. An active session that already contains a provider checkout reference is safely reused; an in-flight session returns `checkout_in_progress` until that reference exists; terminal/expired replay is not silently reopened. Appwrite transaction-backed unique lock/session creation is used for concurrent requests, with conflict retries rather than an assumption of client-side serialization.
...
| Canonical `npm run build` | PENDING CURRENT PR RERUN | The previous local termination/OOM remains confirmed as an environment limitation; the prior higher-resource CI/Vercel build passed before this no-key correction. |
...
The initial implementation commit is `fe5d8be8dad2b4c2d066249323bfaf57147b3075`; the schema/recovery correction is `708faa4012f40a18edf11dd7af5a0f36be7b6505`; the fail-closed authoritative-read correction is `f7da71b9f8c84e92f7736ab7fe7ff5b5e2b59a02`; the no-key idempotency correction is `c94e485612756a032ad710ff0e07916e3005554b`. Draft PR #223 is open and Draft at the new correction head; its current CI/Vercel reruns are pending. No Appwrite deployment, schema application, provider mutation, secret change, DNS change, Vercel configuration change, or payment occurred. Production remains disabled and unverified.
...
The owner review identified three real defects. First, user-scope lock documents omitted fields that were incorrectly required by the shared lock schema; plan-only fields are now optional in the shared schema and required by runtime validation for plan-scope records. Second, authoritative subscription/provider-state read failures were converted to `null`; `findOptional()` now raises sanitized `state_unavailable`, so no reservation, lock, session, or provider call can occur during a partial read failure. Third, omitted idempotency keys shared a 24-hour fallback and could cross-contaminate unrelated no-key plans or block retries after failure; omitted keys now receive unique server-generated attempt keys while explicit-key recovery remains deterministic. Tests cover all corrections and preserve the existing shared resolver precedence. The reused-session path returns the same safe provider checkout reference after a lost original response and returns `checkout_in_progress` while a provider reference is not yet available.

## Exact next action

Keep PR #223 Draft and do not mark it Ready or merge it. Review the no-key idempotency correction after the current CI/Vercel reruns. A later explicit authorization is required before marking Ready, merging, applying the schema, deploying `billing-checkout`, enabling billing, or configuring Production.
