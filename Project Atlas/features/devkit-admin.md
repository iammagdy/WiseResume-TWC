...
* Production-verified runtime observability adds an `AI QA Evidence` panel backed by the signed `admin-devkit-data` action `list-ai-runtime-receipts`. It is read-only and may filter safe receipt metadata by request ID, hub, feature, and state. The panel deliberately never renders prompts, completions, request bodies, raw Appwrite/provider errors, credentials, headers, or raw user identifiers. The `ai_runtime_receipts` schema and its four affected hubs were provisioned/deployed by official workflow `31375728081`; deterministic production receipts for `ai-gateway`, `resume-section-ai`, and `job-import` are verified.
