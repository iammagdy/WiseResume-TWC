...
**Status:** `PAYPAL_REFUND_REVERSAL_PREDEPLOY_READY` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `PENDING_TARGETED_DEPLOYMENT`, Schema: `SCHEMA_DELTA_LIKELY_REQUIRED_BUT_LIVE_STATE_UNVERIFIED`, `PAYPAL_PRODUCTION_READY = NO`, Main Baseline: `84aa793d1cea97a9233caf3b388a4dc3f1fdf61c`, Billing: `CHECKOUT_PREVIOUSLY_VERIFIED_DISABLED` (FRESH_PREDEPLOY_RUNTIME_VERIFICATION_REQUIRED)) — Option B refund & payment reversal policy merged into main via PR #301 (`69b31c0d`): (1) Full refund immediately revokes entitlement, preserves payment identity, sets `renewal_cancellation_pending = true`, and cancels provider renewal; (2) Partial refund preserves access and renewal without local arithmetic; (3) Historical refund/reversal preserves active state using authoritative state/ledger timestamps; (4) Reversal immediately revokes entitlement using `resource.id` as sale ID; (5) New sales blocked during pending cancellation; (6) Fail-closed tombstone lookup with reversal precedence; (7) Direct refund status convergence (COMPLETED/PENDING retryable 503, unknown fail closed 502); (8) True legacy migration-on-touch via provider `start_time` and explicit query range; (9) HATEOAS Transactions API pagination with 5-page safety bound; (10) Frontend neutral copy during pending cancellation. All 134 webhook, 13 schema, 23 coupons, 369 hub, and 30 frontend tests passing. Frontend deployed to production via Vercel; Appwrite schema provisioner hardened with index readiness polling (`waitForIndexAvailable`); live schema state unverified; Appwrite refund/reversal hubs pending targeted deployment (recommended sequence: paypal-webhook then coupons); PayPal Sandbox runtime QA not performed (preferred mechanism: PayPal Sandbox Business Dashboard refund); public checkout previously verified disabled (fresh predeploy runtime verification required); Production PayPal COMPLETELY UNTOUCHED (`PAYPAL_PRODUCTION_READY = NO`). What's New decision: `WHATS_NEW_DEFER_UNTIL_PRODUCTION`. Next action: controlled schema provisioning + targeted paypal-webhook/coupons deployment planning and Sandbox runtime QA, requiring separate owner authorization.
