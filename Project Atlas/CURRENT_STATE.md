...
> **Production Preflight State — 2026-09-10:** Whop Production compatibility hotfix for `invoice.created` completed end-to-end (PR #332, commit `a46c2a80`) and deployed to Appwrite production. Targeted Appwrite deployment `deploy-appwrite-hubs.yml` (Run `34446694829`, target: `whop-webhook`, deployment `6aa252496ce67daf19fb`) reached status `ready`. All 13 subscribed events reported by Whop Production API (including `invoice.created`) are fully supported. `invoice.created` is safely handled as a non-entitlement observational ledger event (`processing_status: 'processed'`, `outcome_code: 'observed_without_entitlement_mutation'`, `mutated: false`), verified on live runtime with HTTP 200. Idempotency verified on duplicate delivery (`outcome: 'duplicate'`, `code: 'already_recorded'`). Unsigned probe verified fail-closed (HTTP 401). Appwrite database integrity verified: zero mutation in `whop_subscription_state`, zero customer entitlement mutation. REAL_PAYMENT_CREATED = NO, REAL_PAYMENT_COMPLETED = NO, CUSTOMER_ENTITLEMENT_MUTATED = NO. Production preflight passed.

**Last Verified:** 2026-09-10
**Status:** `WHOP_PRODUCTION_PREFLIGHT_PASS__READY_FOR_FIRST_LIVE_TRANSACTION` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `billing-checkout`, `whop-webhook`, `coupons`, `ai-gateway`, `admin-devkit-data`: `DEPLOYED_AND_VERIFIED_LIVE`, Whop Production: `ACTIVATED_AND_PREFLIGHT_VERIFIED`, Primary Provider: `whop`, Fallback Provider: `paypal` [frozen], Catalog: `LIVE_RECURRING_MONTHLY`, Webhook Subscriptions: `13_EVENTS_EXACT_MATCH`, DevKit Owner Browser Verification: `DEFERRED_BY_OWNER`)

* **Verified UX Fixes, Activation Improvements & Onboarding Idempotency (2026-09-10):** `VERIFIED_READY` (PR: [#334](https://github.com/iammagdy/WiseResume-TWC/pull/334), Merge Commit: [`f828c47d864d44e22e9d2bf84a4a4bbcf21d7917`](https://github.com/iammagdy/WiseResume-TWC/commit/f828c47d864d44e22e9d2bf84a4a4bbcf21d7917), Vercel Production Deployment: `6373222083`, Live URL: `https://wiseresume.app`, Backend: `UNTOUCHED`, Payments/Billing: `STRICTLY_FROZEN_AND_UNTOUCHED`, Focused Tests: `42_OF_42_PASS`, Full Suite: `1443_PASS__1_PRE_EXISTING_FAIL__BUILD_CLEAN__I18N_PASS`, Live Browser QA: `100%_PASS_ON_PRODUCTION_SURFACES`) — Merged and deployed frontend fixes and activation improvements:
  1. **REG-01 (Arabic Empty State & Alignment):** Added 7 missing localized keys to `locales/en/app.json` and `locales/ar/app.json`. Replaced `mr-1.5` on icon buttons in `EmptyState.tsx` with `gap-1.5` for symmetric LTR/RTL spacing.
  2. **BUG-02 (Editor Silent Bounce):** Updated missing `targetId` redirect in `EditorPage.tsx` to `/dashboard?action=create`, directly opening `CreateResumeDialog`. Added unit test in `EditorRecovery.test.tsx` (2/2 pass).
  3. **BUG-01 (Career Page Error State):** Handled `isError` in `CareerPage.tsx` with neutral error card, `AlertCircle` icon, localized retry, and dashboard return. Added unit tests in `CareerPage.test.tsx` (5/5 pass).
  4. **P1 (Onboarding CREATE Starter Resume & Partial-Write Idempotency):** Added `createStarterResume?: boolean` to `SaveProfileArgs` in `onboardingProfile.ts` and updated `handleManualCreate` in `OnboardingPage.tsx` to pass `createStarterResume: true` and navigate directly to `/editor?id=${result.resumeId}`. Updated `CelebrationStep` to route to `/editor?id=${createdResumeId}`. Added synchronous ref lock preventing double-submit. Resolved partial-write duplicate vulnerability: on step 2 (resume creation) success but step 3 (profile onboarding flag) failure, retains the created `resumeId` for subsequent reconciliation via `reconcileOnboardingCompletion(userId)` rather than discarding it; checks existing resumes before creation ensuring retry idempotency; in `OnboardingPage.tsx` stores `createdResumeIdRef` preventing duplicate resumes on user retry. Unit tests: `onboardingProfile.test.ts` (13/13 pass), `OnboardingPage.test.tsx` (12/12 pass). Production safety: `PRODUCTION_ONBOARDING_LIFECYCLE_NOT_REEXECUTED_FOR_DATA_SAFETY`.
  5. **P1 (In-Editor AI Credit Transparency):** Integrated `useAICredits` and `useLocale` in `AIEnhanceDialog.tsx` to display an authoritative remaining daily quota badge in the actions footer. Safe defaults for optional props. Added unit tests in `AIEnhanceDialog.test.tsx` (5/5 pass).
  6. **P1 (Free AI In-Editor Communication):** Added guidance callout under `UpgradeWall` in `AIStudioPage.tsx` informing Free users that 5 daily AI actions are included directly inside the Resume Editor. Unit tests in `AIStudioPage.test.tsx` (5/5 pass).
  7. **Factual Correction (Pricing Matrix):** Corrected Ultimate daily AI quota in `PricingPage.tsx` comparison table from `"200 / day"` to canonical `"Unlimited"`. Verified live on `https://wiseresume.app/pricing` and `https://wiseresume.app/ar/pricing` (zero occurrences of stale "200" copy).
  8. **SettingsPage Reversion (Classification: UNRELATED_CHANGE):** Reverted branch-local edit in `SettingsPage.tsx` preserving pre-existing main baseline. Full suite failure in `englishUiFallbackCoverage.test.ts` (`settings.tabs.notifications`) proven pre-existing on `origin/main` (`b403181c`).
  - **Live Production Browser QA Evidence:** Executed Playwright QA against live production (`https://wiseresume.app`): Homepage HTTP 200, 0px overflow; Pricing EN HTTP 200, Free ($0), Pro ($5/mo), Ultimate ($10/mo), Unlimited daily AI quota displayed, 0 stale "200" strings, 0px horizontal overflow across desktop (1440x900) and mobile (390x844); Pricing AR HTTP 200, RTL layout active (`dir="rtl"`), zero stale copy, 0px mobile overflow; Auth Health HTTP 200, Appwrite production API reachable (`https://fra.cloud.appwrite.io/v1/health/version` returned HTTP 200), zero CORS / origin errors.

* **Frontend & Product UX Redesign (2026-09-10):** `BROWSER_TESTED_LOCAL_WITH_CONTROLLED_FIXTURES` (Branch: `visual/frontend-product-experience-refresh`, Commits/Pushes: `NONE_LOCAL_WORKING_TREE_ONLY`, Backend: `UNTOUCHED`, Payments/Billing: `STRICTLY_FROZEN_AND_UNTOUCHED`, Browser QA: `198_PASSES__0_FAILURES__0PX_HORIZONTAL_OVERFLOW`) — Comprehensive frontend and product UX refresh across 6 core workspace surfaces and 6 responsive viewports tested locally with controlled fixtures:
  1. **Onboarding Flow (`src/pages/OnboardingPage.tsx`):** Redesigned as goal-first intake (`create`, `improve`, `tailor`). Skip escape strictly hidden on step 1 to enforce deliberate intent; step 2+ provides skip capability. Tailoring prerequisite check guards against zero-resume state by routing users to build a base resume first. Decoupled `OnboardingChecklist` so it persists cleanly in empty states. Applied destructuring bug fix `const { data: resumes = [] } = useResumes();` preventing `TypeError` on goal selection.
  2. **Dashboard Empty State & Populated State (`src/components/dashboard/EmptyState.tsx`, `src/pages/DashboardPage.tsx`):** Replaced 5-tier stacked layouts with a calm, single-composition card featuring exactly 2 primary CTAs ("Create Resume" and "Upload Existing Resume"), ATS/AI capability benefit pills, and contextual checklist slot. Resolved missing import for `DashboardDiscoverySection` in `DashboardPage.tsx` preventing runtime `ReferenceError` in populated dashboard.
  3. **Remote Jobs Feed (`src/pages/RemoteJobsPage.tsx`, `src/components/jobs/JobCard.tsx`, `src/hooks/useRemoteJobs.ts`):** Consolidated 20+ role groups into 7 top-level display pills (`All`, `Easy & Entry`, `Marketing & Content`, `Support & Admin`, `Tech`, `Design & Creative`, `More`). Redesigned `JobCard` to prioritize Title → Company → Location → Date → Salary while removing raw text clutter. Introduced full Job Details dialog, Fast Tailor integration, and mobile filters bottom sheet (`JobFiltersSheet.tsx`). Migrated to TanStack Query v5 `keepPreviousData` caching.
  4. **Settings Workspace (`src/pages/SettingsPage.tsx`, `src/components/settings/SettingsTabLayout.tsx`):** Converted navigation to sticky horizontal tabs (`account`, `preferences`, `notifications`, `privacy`, `help`) with deep linking (`?tab=`, `?changelog=true`). Built real-time unified client-side search across all sections with category pills and keyboard shortcut (`Cmd+K`/`Ctrl+K`). Full Arabic coverage verified (13/13 surfaces).
  5. **Subscription & Pricing Presentation Layer (`src/pages/SubscriptionPage.tsx`, `src/pages/PricingPage.tsx`):** Removed 560 lines of dead unreachable JSX. Realigned presentation copy (`200 AI actions/day` for Ultimate) with canonical `planConfig.ts` limits. 3-tier SaaS pricing cards, golden Recommended badge, FAQ accordion, and 9-feature comparison matrix verified. All billing tokens, payment webhooks, and payment logic strictly frozen and untouched.
  - **Browser QA Matrix (198/198 PASS, 0 FAIL, 0px Overflow):** Automated Playwright test suite executed across 6 surfaces and 6 viewports (Desktop EN 1280x800, Desktop AR Dark 1280x800, Tablet EN 768x1024, Mobile EN 390x844, Mobile AR Dark 390x844, Short Mobile EN 390x600) using local Chromium and controlled Appwrite test fixtures.
  - **Validation & Build Integrity:** `npx tsc --noEmit` clean (0 errors), `npm run build` clean (Vite 44.8s, 0 sourcemaps), `tests/build/performance-build-contract.test.cjs` pass (3/3), `npm run test:i18n` pass (11/11 namespaces), `npm run test:i18n:coverage` pass (13/13 surfaces), Vitest test suites pass (59/59 tests across 7 test files).
* **Whop invoice.created Compatibility Hotfix & Preflight Pass (2026-09-10):** `WHOP_PRODUCTION_PREFLIGHT_PASS__READY_FOR_FIRST_LIVE_TRANSACTION` — Resolved Whop provider-side webhook delivery failure caused by `invoice.created` inclusion: (1) Added `invoice.created` to `SUPPORTED_EVENTS` in `appwrite-hubs/whop-webhook/src/main.js` while strictly keeping it out of `STATE_EVENTS`; (2) Ensured non-entitlement observational ledger behavior (`processing_status: 'processed'`, `outcome_code: 'observed_without_entitlement_mutation'`, `mutated: false`) with zero writes to `whop_subscription_state`; (3) Added Scenarios K through N to `tests/hubs/whop-webhook.test.cjs` covering acceptance, ledger recording, zero state mutation, idempotency, unknown event rejection, and production mode; (4) Recomputed DevKit source hashes (`whop-webhook`: `c5fcc9f4...`); (5) Passed all tests, TypeScript typecheck, and Vite production build; (6) Merged PR #332 (`a46c2a80`) to `main`; (7) Executed targeted Appwrite deployment `deploy-appwrite-hubs.yml` (Run `34446694829`, target `whop-webhook`), reaching status `ready` (`6aa252496ce67daf19fb`); (8) Verified live endpoint `https://whop-webhook.wiseresume.app`: unsigned probe returned HTTP 401 fail-closed, signed unknown event probe returned HTTP 400 `invalid_event`, signed `invoice.created` probe returned HTTP 200 `observed_without_entitlement_mutation`, duplicate probe returned HTTP 200 `already_recorded`; (9) Appwrite database verified: `whop_subscription_state` count is 0 (zero mutation), `whop_event_ledger` accurately recorded observational entry; (10) Authenticated Whop Production API verified: company `biz_B7fMXLLj18wv8J` active, Pro `plan_4JJSQLj5zEKVn` $5/mo active, Ultimate `plan_kt5MScAplbCuN` $10/mo active, webhook `hook***hmQv` enabled with exact 13-event match (0 missing, 0 unsupported); (11) ZERO financial transactions executed. Ready for first live payment.
* **Whop Production Activation & Runtime Cutover (2026-09-09):** `WHOP_PRODUCTION_ACTIVATED__PENDING_FIRST_LIVE_TRANSACTION` — Full production cutover end-to-end: (1) Configured GitHub repository variables (`WHOP_PRODUCTION_COMPANY_ID: biz_B7fMXLLj18wv8J`, `WHOP_PRODUCTION_PRODUCT_ID: prod_WrbEGZdSaG2af`, `WHOP_PRODUCTION_PRO_PLAN_ID: plan_4JJSQLj5zEKVn`, `WHOP_PRODUCTION_PREMIUM_PLAN_ID: plan_kt5MScAplbCuN`, `BILLING_CHECKOUT_PROVIDER: whop`); (2) Hardened deployment scripts and hub runtimes to preserve sandbox secrets and inject authoritative production catalog fallbacks (`deploy_hubs.cjs`, `deploy-appwrite-hubs.yml`, `billing-checkout`, `whop-webhook`, `shared-subscription-resolver`); (3) Updated frontend `billingCheckout.ts` to route `wiseresume.app` to Whop production checkout; (4) Passed all 15 frontend unit tests, 33 hub contract tests, full TypeScript typecheck, and production Vite build; (5) Merged PR #330 (`e81434a9`) to `main`; (6) Vercel production deployed clean (`FikbLTS3nb521d3VrLsJr7Lg9nkL`); (7) Dispatched targeted Appwrite deployment `34403183896` with all 4 hubs (`billing-checkout`, `whop-webhook`, `coupons`, `ai-gateway`) reaching `ready`; (8) Verified live endpoints: `https://whop-webhook.wiseresume.app` returned HTTP 401 fail-closed signature verification, `https://wiseresume.app/subscription` returned HTTP 200; (9) Zero financial transactions executed; (10) Owner recorded `DEVKIT_OWNER_BROWSER_VERIFICATION = DEFERRED_BY_OWNER`.
* **DevKit Billing Intelligence Truthfulness Hotfix (2026-09-09):** `VERIFIED_READY` — Corrected truthfulness gaps from PR #328: (1) Enforced strict evidence-based payment confirmation (Whop `payment.succeeded`, PayPal capture/sale completed, RevenueCat unavailable); (2) Deduplicated global user plan statistics to guarantee every user belongs to at most one effective plan bucket; (3) Gated provider classification and warning dialogs on resolver candidate validity; (4) Reconciled precedence documentation to reflect canonical tier-based precedence (`premium > pro > free`); (5) Reclassified legacy premium audit anomalies with zero manual changes required; (6) Clarified browser QA boundaries (`MOCKED_PRODUCTION_BUNDLE_QA` vs `LIVE_PRODUCTION_INTEGRATION_QA`). Merged via PR #329 (`c7524f89`); targeted Appwrite deployment `34387415399` deployed `admin-devkit-data`; live Appwrite audit `34387637729` confirmed `ENTITLEMENT_DATA_MUTATION = NONE`.
* **DevKit Billing & Entitlements 2026 Refresh (2026-09-09):** `VERIFIED_READY` — Reconciled DevKit/Admin dashboard with multi-provider architecture (Whop Sandbox QA, PayPal alternative, RevenueCat legacy, manual grants, trials, coupons) while decoupling public display (`Ultimate`) from internal canonical database/API keys (`premium`). Merged to `main` via PR #328 (`bf6fc37b`). Vercel production deployed and verified live; targeted Appwrite `admin-devkit-data` deployment active; live Appwrite audit confirmed `ENTITLEMENT_DATA_MUTATION = NONE`; browser QA confirmed `Ultimate` labeling, source pills, and active-provider warning modal.
* **Final Same-Day Closeout (2026-09-09):** `VERIFIED_READY` — Fixed pre-existing billing contract tests (PR #327, SHA `1e0aa83b`), retired TestSprite permanently (PR #326, SHA `c271947b`), and resolved all 4 manual QA regressions for replay keys, scrolling, Free cancellation guard, and signup plan sanitization (PR #325, SHA `941168a7`). Vercel production deployed and verified live; targeted Appwrite `coupons` deployment active; live Appwrite read-only audit confirmed active Whop Pro entitlement; full browser QA passed.
* **TestSprite Permanent Removal & Retirement (2026-09-09):** `TESTSPRITE_REMOVED__NO_LONGER_PART_OF_WISERESUME_CI` — Permanently removed TestSprite from WiseResume per explicit project owner decision. TestSprite is no longer a required check, no longer runs on PRs or push events, and has zero workflow or code dependencies. First-party Playwright and Vitest suites remain the canonical and authoritative automated testing framework.
* **PayPal Appwrite 36-Char Schema Key Limit Fix (Historical) (2026-09-07):** `PAYPAL_SCHEMA_KEY_LIMIT_FIX_READY_FOR_REVIEW` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `PENDING_TARGETED_DEPLOYMENT`, Schema: `PARTIAL_SCHEMA_MUTATION_POSSIBLE_UNVERIFIED`, `PAYPAL_PRODUCTION_READY = NO`, Main Baseline: `0542db3f246b8d3ece002d5034858e9f831a3d5e`) — First authorized targeted deployment attempt (`deploy-appwrite-hubs.yml` run `34107206440`, target: `paypal-webhook`) passed target validation, source-hash gate, and PayPal Sandbox bootstrap, but failed during `Ensure PayPal subscription schema` (`APPWRITE_SCHEMA_KEY_LIMIT`) because attribute identifier `last_entitlement_payment_timestamp_ms` was 37 characters (exceeding Appwrite's 36-character maximum). Deployment of `paypal-webhook` was safely skipped. Live schema status is `PARTIAL_SCHEMA_MUTATION_POSSIBLE_UNVERIFIED` as earlier schema attributes in `setup_paypal_schema.cjs` may have been created before the failure. Remediation on branch `fix/paypal-appwrite-key-limit`: (1) Renamed persisted attribute and runtime property to `last_entitlement_payment_ts_ms` (31 characters); (2) Added pre-mutation schema-key identifier validation (`APPWRITE_KEY_REGEX = /^[A-Za-z0-9][A-Za-z0-9._-]{0,35}$/`) in `scripts/setup_paypal_schema.cjs` before any remote database calls; (3) Updated all 40 occurrences in webhook tests and contract tests (15/15 schema tests passing); (4) Recomputed `paypal-webhook` DevKit source hash (`b6500e96...`). All 134 webhook, 15 schema, 23 coupons, 371 hub, and 30 frontend tests pass. Production PayPal completely untouched. Next action: code review and merge of key limit fix PR.
* **PayPal Refund & Reversal Pre-Deployment Readiness Hardening (2026-09-07):** `PAYPAL_REFUND_REVERSAL_PREDEPLOY_READY` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `PENDING_TARGETED_DEPLOYMENT`, Schema: `SCHEMA_DELTA_LIKELY_REQUIRED_BUT_LIVE_STATE_UNVERIFIED`, `PAYPAL_PRODUCTION_READY = NO`) — Merged PR #301 into `main` at merge SHA `69b31c0d4c762f8650435390ce92ba2e030e19b4` (approved head `45f24dfc17719da8334615ab617e8d642c5b1942`). Option B policy implementation and provider-contract hardening: (1) Full refund immediately revokes entitlement (`expires_at = null`), preserves payment identity, sets `renewal_cancellation_pending = true`, and initiates server-side provider cancellation; (2) Partial refund preserves entitlement and renewals without local arithmetic (`partial_refund_recorded`); (3) Historical refunds/reversals leave active state untouched (`historical_refund_ignored` / `historical_reversal_ignored`), using authoritative state/ledger sale payment timestamps rather than provider `tx.time` or webhook arrival time; (4) Reversal immediately revokes entitlement while preserving truthful provider status; reversal payment identity uses `resource.id` as the affected sale transaction `paymentId` (`resource.parent_payment` captured as non-entitlement parent reference metadata); (5) `PAYMENT.SALE.COMPLETED` blocks entitlement restoration while cancellation is pending; (6) Fail-closed tombstone lookup prevents stale sale reactivation; verified reversal tombstone takes strict precedence over refund (`reversal > refund`) dropping activation (`sale_already_refunded`) without calling Transactions API; verified refund tombstone with unconverged `COMPLETED` Transactions API status fails closed as retryable 503 (`provider_state_not_converged`); `PARTIALLY_REFUNDED` allows normal sale activation; missing schema attributes/indices during tombstone lookup fail closed as retryable 503 (`tombstone_lookup_failed`); conflicting tombstone identity fails closed (`ambiguous_payment_ledger_correlation`); (7) Direct refund convergence: `COMPLETED` and `PENDING` fail closed as retryable 503 (`provider_state_not_converged`); unknown/unexpected provider statuses fail closed as retryable 502 (`unsupported_provider_transaction_status`, zero entitlement mutation, not permanently 2xx-ignored); (8) True legacy migration-on-touch: when existing state documents lack payment identity and old ledger lacks `payment_id`, the canonical subscription snapshot (`GET /v1/billing/subscriptions/{id}`) provides authoritative `start_time` to query the Transactions API (`start_time` to `nowMs`), populating payment identity without bulk backfill; (9) HATEOAS Transactions API pagination with fail-closed behavior; (10) Frontend suppresses active free copy during pending cancellation. Environment isolation is enforced by the hard Sandbox runtime gate. Backend (134/134), frontend (30/30), schema (13/13), and all hubs (369/369) tests passing. Frontend deployed to production via Vercel; Appwrite schema provisioner hardened with index readiness polling (`waitForIndexAvailable`); live schema state unverified; targeted deployment pending (RUN 1: paypal-webhook, RUN 2: coupons); Sandbox refund QA planned via PayPal Sandbox Business Dashboard; Production PayPal untouched. What's New decision: `WHATS_NEW_DEFER_UNTIL_PRODUCTION`. Next action: owner-authorized targeted deployment and Sandbox runtime QA.
* **PayPal Failed-Renewal Local Contract Boundary Hardening (2026-09-06):** `PAYPAL_FAILED_RENEWAL_LOCAL_CONTRACT_FULLY_TESTED` (`PAYPAL_PRODUCTION_READY = NO`) — Branch `docs/paypal-failed-renewal-local-contract-closeout` (Target: `main`): PR #299 merged to `main` at `84a5f005` (+60 lines, -0 lines in `tests/hubs/paypal-subscription-resolver.test.cjs`). Test-only hardening added: (a) Case 20 proves `nowMs === G` drops to `free` when no other entitlement exists (`isFutureTimestamp` uses strict `parsed > nowMs`). (b) Case 21 proves expired PayPal grace does not force Free when another valid entitlement exists (active RevenueCat Pro or Manual/Admin Pro cleanly wins). Full local test suite passes (21/21 resolver, 76/76 webhook, 21/21 coupons, 299/299 hubs). Official PayPal Developer documentation audit verified that on-demand recurring renewal triggers, clock acceleration, and deterministic recurring decline controls have `NO_DOCUMENTED_METHOD` in PayPal Sandbox (PayPal official documentation exposes no documented method to deterministically force a scheduled recurring renewal decline in Sandbox; PayPal-Mock-Response / documented negative-testing mechanisms do not constitute genuine provider-generated recurring renewal lifecycle proof). Authentic natural-expiry verification requires waiting for PayPal's real scheduled billing lifecycle to emit a genuine `BILLING.SUBSCRIPTION.PAYMENT.FAILED` event, then waiting until the WiseResume grace boundary $G = \text{eventTimestamp} + 48\text{ hours}$; provider-side time from subscription creation to the genuine failure event is not deterministic for this QA plan. Webhook Simulator events are synthetic and not verifiable through `POST /v1/notifications/verify-webhook-signature`; provider-runtime failed-renewal lifecycle remains `NOT VERIFIED` (`FAILED_RENEWAL_FULL_SANDBOX_RUNTIME_VERIFIED = NO`). Zero production logic modified; zero deployments; public checkout fail-closed; Production PayPal untouched. What's New: `WHATS_NEW_NOT_REQUIRED`. Next workstream: `PAYPAL_REFUND_REVERSAL_POLICY`.
* **PayPal Cancellation Paid-Through Fix & Runtime E2E Re-Verification (2026-09-06):** `PAYPAL_CANCELLATION_FIX_RUNTIME_VERIFIED_SANDBOX` (`PAYPAL_PRODUCTION_READY = NO`) — Branch `docs/paypal-cancellation-fix-runtime-verification` (Target: `main`): PR #294 merged to preserve paid access through the PayPal cancellation period; PR #295, #296, #297 merged to harden webhook activation supersession, ledger reclaim of recoverable ignored events, and initial payment timestamp handling. Runtime E2E verified in live US PayPal Sandbox with designated Sandbox QA user and verified Ultimate Sandbox subscription. Customer UI cancellation performed on `https://wiseresume.app/subscription`. Direct PayPal REST API confirmed `CANCELLED` at `2026-09-06T10:59:18Z`. First automatic PayPal webhook event `BILLING.SUBSCRIPTION.CANCELLED` delivered and processed cleanly with PayPal POSTBACK signature verification returning SUCCESS (`POST /v1/notifications/verify-webhook-signature`). Note: Real provider-generated activation and payment events were successfully processed after provider re-delivery during runtime recovery. Appwrite `paypal_subscription_state` and coupons resolver verify `status = canceled`, `expires_at = 2026-10-06T10:00:00.000Z` (PRESERVED), `effective_plan = premium` (unlimited AI quota retained until Oct 6, 2026; server-side AI entitlement follows unchanged resolver contract). Live UI verified: "Ultimate [Canceled]" with notice "Access ends on October 6, 2026" and cancel button removed. Public checkout fail-closed restored (`BILLING_CHECKOUT_ENABLED=false`, `BILLING_CHECKOUT_PROVIDER_READY=false`) and deployed; HTTP 403 `payments_disabled` verified. All 180 backend and 28 frontend unit/integration tests pass. What's New decision: `WHATS_NEW_DEFER_UNTIL_PRODUCTION`. Governance deviation: `OWNER_AUTHORIZATION_WORKFLOW_VIOLATION` documented. Pre-existing gap retained: `BILLING_CHECKOUT_DEVKIT_SOURCE_HASH_NOT_TRACKED_PRE_EXISTING`. Pending blockers: failed renewal/48h grace, refunds/reversals, and Live rollout.

* **PayPal US Sandbox Pro & Ultimate E2E Verified (2026-09-06):** `US_SANDBOX_PRO_AND_ULTIMATE_E2E_VERIFIED` — Branch `docs/paypal-us-sandbox-ultimate-e2e-closeout` (Target: `main`): (1) Migrated runtime Sandbox merchant catalog to an owner-approved US-based PayPal Sandbox Business App. (2) Pro E2E subscription executed with real $5.00 USD payment completion. (3) Ultimate E2E subscription executed with real $10.00 USD payment completion. (4) Real provider-generated lifecycle events (`BILLING.SUBSCRIPTION.ACTIVATED` and `PAYMENT.SALE.COMPLETED`) were successfully processed after provider re-delivery following targeted synchronization of the active Sandbox QA webhook fixture into `paypal-webhook`. Cryptographic PayPal POSTBACK signature verification returned `SUCCESS`. (5) Appwrite `paypal_event_ledger` and `paypal_subscription_state` verified active (`plan = pro` for Pro, `plan = premium` for Ultimate, `status = active`, `environment = sandbox`). (6) Authoritative subscription resolver resolves `premium` (unlimited AI credits in `ai-gateway`) and `pro` (50 AI credits/day). (7) Live Subscription page UI verified; tier persists across reloads and navigation. (8) Non-QA isolation verified (HTTP 403 `payments_disabled`). (9) Public checkout restored to fail-closed (`BILLING_CHECKOUT_ENABLED=false`, `BILLING_CHECKOUT_PROVIDER_READY=false`). (10) Production PayPal untouched; cancellation, failed renewal, and refunds remain not tested (`READY_FOR_CANCELLATION_QA = YES`). What's New decision: `WHATS_NEW_DEFER_UNTIL_PRODUCTION`.
* **What's New Page Full Revamp & Shipped-History Reconciliation (2026-09-03):** `WHATS_NEW_REVAMP_DEPLOYED_PRODUCTION_VERIFIED` — Transformed `/whats-new` and `/ar/whats-new` from an un-scannable 9,427px chronological feed into a modern, responsive release hub with monthly progressive disclosure. Merged via PR #280 (`59d38309dbd2bb68ee14cf913cf3d9192eb2b013`) and deployed to production on Vercel (`Hc26exYbknDjyKee7TZH3qDbRsJw`). Real production browser QA verified across all 8 matrix combinations on `https://wiseresume.app/whats-new` and `https://wiseresume.app/ar/whats-new` (Desktop 1440px & Mobile 390px x EN LTR & AR RTL x Light & Dark) with HTTP 200, 0 console errors, 0 page errors, and 0px horizontal overflow. Initial page scroll height materially reduced: Desktop EN 5,011px (vs old 9,427px, 46.8% reduction), Desktop AR 4,857px (vs old 9,427px, 48.5% reduction), Mobile EN 7,174px (58.8% reduction vs prior uncollapsed redesign), Mobile AR 6,459px (58.3% reduction). Architecture verified live: Latest Highlights (top 3 September releases: More Reliable Native PDF Exports, Asynchronous LinkedIn Profile Optimization, and Instant Tailoring Cancellation) with feed deduplication (September feed renders compact reference banner and 3 remaining non-featured updates in default view; filters automatically expose all matching cards). Monthly progressive disclosure verified live: September and August 2026 expanded by default; older months (July 2026 through October 2025) collapsed by default behind accessible semantic button controls (`aria-expanded`, `aria-controls`, `aria-label`, visible focus). Expand/collapse toggling verified live on July 2026 (expands on click, reveals 4 articles, collapses on second click). Dual Type (`All` | `New` | `Improved` | `Fixed`) and Product Area (`All Areas` | `Resume & Portfolio` | `AI & Tailoring` | `Jobs & Career` | `Platform` | `Security & Legal`) filters verified live with dynamic auto-expansion of matching months (Fixes filter isolates 5 items; Security & Legal filter isolates 3 items). Strictly qualified public customer copy verified live on production: 0 occurrences of banned overclaims (`Across All Templates`, `pixel-perfect`, `over 90% bandwidth reduction`, `battery`, `Guaranteed protection`, `genuine recruiter`, `Chromium`). Assigned explicit `updateType: 'new' | 'improved' | 'fixed'` across all 40 releases (19 new, 16 improved, 5 fixed). Zero Appwrite backend or schema changes required (`APPWRITE_PRODUCT_SCHEMA_CHANGE = NO`). Billing checkout remains preserved as disabled (`BILLING_CHECKOUT_DISABLED`).
...
* **Verdict:** `PAYPAL_PHASE3_FINAL_TESTED_LOCAL_READY_TO_COMMIT`. Dedicated server-only Appwrite Function `appwrite-hubs/paypal-webhook` registered in canonical Appwrite deployment contract (`appwrite.json`, `appwrite-function-policy.cjs`, `deploy_hubs.cjs`, `deploy-appwrite-hubs.yml`), deployment workflow secrets wired, non-mutating preflight validation before schema mutation (`scripts/validate_paypal_bootstrap.cjs`), explicit fail-closed environment contract (zero implicit sandbox default; missing/empty/invalid/production fail closed), safe two-stage bootstrap contract implemented (`REQUIRED_FOR_BOOTSTRAP` vs `REQUIRED_FOR_WEBHOOK_ACTIVATION`), webhook-ID anti-downgrade rule verified, lockfile committed, preflight validated, cryptographic webhook signature verification via PayPal REST API, canonical user correlation with checkout session bridge, atomic concurrency reservation with hard-crash/timeout recovery, Sandbox QA mutation boundary, equal-timestamp ordering, UPDATED non-elevation policy, hard Sandbox-only runtime gate, deterministic state machine, and 48-hour failed payment grace period implemented and verified locally via 103 automated test cases across 12 test suites.
* **Git & Deployment Status:** Branch `feat/paypal-sandbox-phase3`, **NOT COMMITTED**, **NOT PUSHED**, **NOT DEPLOYED**. Zero Appwrite changes applied, conditional workflow schema hook added but NOT executed, zero PayPal webhooks registered, zero PayPal webhook IDs created, zero PayPal transactions executed.
...
* **Safety Boundary:** PayPal Sandbox entitlements and state mutations strictly restricted to `BILLING_CHECKOUT_QA_USER_ID` with canonical ownership check (`currentCanonicalUserId === QA_USER_ID && state.user_id === currentCanonicalUserId`). Unconfigured or mismatched QA users fail closed to Free without state mutation. Production PayPal remains strictly disabled; live billing checkout remains disabled (`BILLING_CHECKOUT_DISABLED`).
* **OWNER_ACTION_REQUIRED:** Provision Sandbox secrets (`PAYPAL_SANDBOX_CLIENT_ID`, `PAYPAL_SANDBOX_CLIENT_SECRET`, `BILLING_CHECKOUT_QA_USER_ID`) before initial bootstrap deployment (Stage A); register Appwrite HTTPS URL in PayPal Developer Dashboard and provision `PAYPAL_SANDBOX_WEBHOOK_ID` after initial deployment for webhook activation (Stage B).
...
## PayPal Sandbox Integration Phase 2 (Data Layer & Entitlement Resolution) — 2026-09-03 (PR #285 MERGED @ 420429d0)

* **Verdict:** `MERGED_TO_MAIN`. Additive data layer, idempotent Appwrite schema script, provider environment isolation (`PAYPAL_ACCESS_ENVIRONMENT`), canonical QA-user ownership boundary, and same-rank precedence preservation merged to `main` via PR #285 (`420429d034c50fddc3eba2677a64dfabb28702cb`).
