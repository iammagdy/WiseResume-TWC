...
* **Coupon Live Legacy Attributes Compatibility Fix (2026-09-07):** `COUPON_LIVE_LEGACY_ATTRS_COMPAT_READY_FOR_REVIEW` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `paypal-webhook`: `DEPLOYED_SANDBOX` [run 34110889444], `coupons`: `PENDING_TARGETED_DEPLOYMENT`, PayPal Schema: `READY`, Coupon Schema Legacy Compat: `IN_REVIEW`, `PAYPAL_PRODUCTION_READY = NO`, Base Main SHA: `a88007427b47c88def31de60883d18ca85095255`) — Targeted deployment attempt for `coupons` (run `34114192398`) succeeded on `discount_codes.code` but failed during `Ensure coupon security schema` with `Incompatible attribute "discount_codes.active": required false (expected true), default null (expected true)`. Historical Appwrite collection provisioned `discount_codes.active` as optional boolean with null default, and `percent_off` similarly has historical optional integer state. Implemented narrow non-destructive legacy tolerance in `scripts/setup_discount_codes_schema.cjs` for proven live Appwrite collection states: (1) `discount_codes.active` accepts `required=false` or `true` and `default=null`, `true`, or `false`; (2) `discount_codes.percent_off` accepts `required=false` or `true` and `default=null`, `100`, or integer; (3) `discount_codes.code` accepts string size 50 or 64 (`required=true`); (4) `coupon_redemptions.user_id` accepts optional status (`required=false` or `true`) and size >= 64 (preserving live size 65000). All other attributes and all indexes remain strictly fail-closed. Expanded `tests/hubs/coupon-schema.test.cjs` with 18 contract assertions. All 16 coupon schema and 384 hub tests pass. Zero business logic changed; source hashes untouched; Production PayPal untouched.
