...
> **Current branch note — 2026-09-08:** `feat/whop-payments-integration` contains the local Whop-primary/PayPal-alternative recurring-only checkout finalization. It is validated locally but is not Production-activated. Whop Sandbox provider lifecycle verification is still pending; the existing PayPal Production-isolated state remains unchanged.
...
* **PayPal Sandbox Payment Core Final Verification & Closeout (2026-09-07):** `SANDBOX_PAYMENT_CORE_VERIFIED_READY_FOR_PRODUCTION_ACTIVATION` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `paypal-webhook`: `DEPLOYED_SANDBOX` [run `34118592362`], `coupons`: `DEPLOYED_OR_READY`, PayPal Schema: `READY`, `PAYPAL_PRODUCTION_READY = NO`, Base Main SHA: `9c27773f4bc7c69d42a53cb25d83e6a0a471b316`) — Merged PR #309 into `main` at `9c27773f` implementing Step 3 provider Sale fallback (`fetchSaleDetails`) and narrowed rejected-event redelivery reclaim. Deployed `paypal-webhook` via targeted GitHub Actions run `34118592362` (`ready`, deployment `[verified deployment]`). Redelivered authentic Sandbox refund event `[genuine Sandbox refund event]`. Webhook reclaimed reservation, resolved subscription `[Sandbox QA subscription]`, populated payment identity via Case C migration-on-touch, and executed Option B full refund revocation (`effective_plan: "free"`, `expires_at: null`, `status: "canceled"`). Authenticated live browser QA on `https://wiseresume.app/subscription` confirmed Free tier rendering, quota display (0/1 resumes, 0/5 AI credits), disabled public checkout notices, and persistent state across page reloads and route navigation.
* **PayPal Legacy Refund Correlation Fix & Redelivery Reclaim (Historical) (2026-09-07):** `PAYPAL_LEGACY_REFUND_CORRELATION_FIX_READY_FOR_PR` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `paypal-webhook`: `DEPLOYED_SANDBOX` [run 34110889444], `coupons`: `DEPLOYED_OR_READY`, PayPal Schema: `READY`, `PAYPAL_PRODUCTION_READY = NO`, Base Main SHA: `55087d29f8840fcdcda38712eddca9ae77614736`) — Diagnosed and resolved authentic refund correlation defect where PayPal's authentic `PAYMENT.SALE.REFUNDED` webhook resource has `sale_id` but lacks `billing_agreement_id`. For pre-PR #299 legacy subscriptions without `last_entitlement_payment_id`, event was rejected with `unresolved_subscription_correlation` before reaching Case C ("True Legacy Migration-on-Touch"), and subsequent redeliveries were permanently blocked as duplicate `already_recorded`. Added Step 3 provider `fetchSaleDetails` fallback, identity conflict guards, and narrowed rejected correlation event reclaim. 149/149 webhook tests and 399/399 hub tests pass.
* **Coupon Live Legacy Attributes Compatibility Fix (Historical) (2026-09-07):** `COUPON_LIVE_LEGACY_ATTRS_COMPAT_READY_FOR_REVIEW` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `paypal-webhook`: `DEPLOYED_SANDBOX` [run 34110889444], `coupons`: `PENDING_TARGETED_DEPLOYMENT`, PayPal Schema: `READY`, Coupon Schema Legacy Compat: `MERGED` [PR #308 at `55087d29`], `PAYPAL_PRODUCTION_READY = NO`, Base Main SHA: `a88007427b47c88def31de60883d18ca85095255`) — PR #308 merged into `main` at `55087d29` implementing narrow legacy tolerance in `scripts/setup_discount_codes_schema.cjs` for `discount_codes.active` and `discount_codes.percent_off`.
* **Coupon Live Schema Compatibility Fix (Historical) (2026-09-07):** `COUPON_LIVE_SCHEMA_COMPAT_READY_FOR_REVIEW` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `paypal-webhook`: `DEPLOYED_SANDBOX` [run 34110889444], `coupons`: `PENDING_TARGETED_DEPLOYMENT`, PayPal Schema: `READY`, Coupon Schema Compat: `MERGED` [PR #307 at `a8800742`], `PAYPAL_PRODUCTION_READY = NO`, Base Main SHA: `ac8172fa226d5eadc29163b54edf5511463e6575`) — PR #307 merged into `main` at `a8800742` implementing narrow legacy tolerance in `scripts/setup_discount_codes_schema.cjs` for `discount_codes.code` (size 50 or 64) and `coupon_redemptions.user_id` (size >= 64, optional).
* **PayPal Appwrite Targeted Deployment & Schema Key Limit Fix (2026-09-07):** `PAYPAL_WEBHOOK_DEPLOYED_SANDBOX` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `paypal-webhook`: `DEPLOYED_SANDBOX`, Schema: `READY`, `coupons`: `PENDING_TARGETED_DEPLOYMENT`, `PAYPAL_PRODUCTION_READY = NO`, Main SHA: `2341c263a1570f94dbafd80e9e3c901b87d366eb`) — PR #305 merged into `main` at `2341c263` fixing the 36-character attribute key limit (`last_entitlement_payment_ts_ms`) and letter-first Appwrite identifier validation. Targeted deployment of `paypal-webhook` via `deploy-appwrite-hubs.yml` (run `34110889444`) succeeded with status `READY` (ready deployment `[previous deployment]`); PayPal collections `paypal_subscription_state` and `paypal_event_ledger` reconciled and verified in Appwrite; safe smoke check returned HTTP 400 (fail-closed pass).
