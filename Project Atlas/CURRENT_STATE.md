...
* **Deployment boundary:** Appwrite deployment remains `NOT_DEPLOYED`. Runs `31871663976` and `31875957559` both failed before deployment at the `ai_runtime_receipts` schema assertion with `permissionsIsArray=false, permissionCount=unknown, documentSecurity=false`; no function was deployed. The root cause was a repository/model-shape bug: the assertion read `collection.permissions`, while Appwrite Collection responses expose `$permissions`. The merged PR #189 fix reads `$permissions`, preserves server-only fail-closed semantics, and adds no mutation or permission-broadening path. Vercel production remained `READY`; no manual Vercel deployment was performed.
