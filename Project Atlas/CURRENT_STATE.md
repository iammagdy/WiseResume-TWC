...
**Status:** `US_SANDBOX_ULTIMATE_10_READY_FOR_QA` (Branch: `fix/paypal-us-sandbox-ultimate-10`, Target: `main`, Audit Baseline: `FULL_FUNCTIONALITY_AUDIT_P1_CLOSEOUT_COMPLETE`, Billing: `BILLING_CHECKOUT_DISABLED`) — PayPal US Sandbox integration verified: (1) US Sandbox merchant app (`PROD-1XU04121YA801240V`) approved & configured. (2) Pro ($5.00/mo) real Free -> Pro subscription (`I-16N4WPDDPWJW`) with real $5 USD payment E2E VERIFIED. (3) Real cryptographic webhook signatures (`BILLING.SUBSCRIPTION.ACTIVATED`, `PAYMENT.SALE.COMPLETED`) verified with postback `SUCCESS`. (4) Appwrite `paypal_event_ledger` and `paypal_subscription_state` verified active for QA user `qa_pp_a91504a8`. (5) Effective plan resolver & `ai-gateway` verified (50 AI credits/day). (6) UI persistence verified on live subscription page. (7) US Sandbox Ultimate plan (`P-56D04005HN592501XNKN6I3Q`) aligned from $15/mo to approved commercial contract of **$10.00 USD/month** via PayPal in-place pricing update (`PRICE_UPDATED`). (8) Ultimate real E2E remains NOT TESTED. (9) Production PayPal UNTOUCHED; public checkout DISABLED (`BILLING_CHECKOUT_ENABLED=false`). (10) What's New decision: `WHATS_NEW_DEFER_UNTIL_PRODUCTION`.
* **PayPal US Sandbox Pro E2E Verified & Ultimate $10 Price Alignment (2026-09-05):** `US_SANDBOX_ULTIMATE_10_READY_FOR_QA` — Branch `fix/paypal-us-sandbox-ultimate-10` (Target: `main`): (1) Migrated runtime Sandbox merchant catalog to an owner-approved US-based PayPal Sandbox Business App (`PROD-1XU04121YA801240V`) to eliminate recurring payment decline issues associated with the prior Egypt Sandbox merchant. (2) Pro E2E subscription `I-16N4WPDDPWJW` executed with real $5.00 USD payment completion in Microsoft Edge automation using verified US buyer account (`sb-bewbg52785550@personal.example.com`). (3) Cryptographic webhook postback signature verification (`SUCCESS`) confirmed for real PayPal Sandbox lifecycle events (`BILLING.SUBSCRIPTION.ACTIVATED` and `PAYMENT.SALE.COMPLETED`) delivered to `https://paypal-webhook.wiseresume.app`. (4) Appwrite `paypal_event_ledger` and `paypal_subscription_state` verified active (`plan = pro`, `status = active`, `environment = sandbox`) for QA user `qa_pp_a91504a8`. (5) Authoritative subscription resolver resolves `pro` (50 AI credits/day quota in `ai-gateway`). (6) Live Subscription page UI verified; Pro tier persists across reloads and navigation. (7) US Sandbox Ultimate plan (`P-56D04005HN592501XNKN6I3Q`) aligned from $15/mo to approved commercial contract of **$10.00 USD/month** via PayPal in-place pricing update (`PRICE_UPDATED`). (8) Ultimate real E2E remains NOT TESTED; Production PayPal untouched; public checkout disabled. What's New decision: `WHATS_NEW_DEFER_UNTIL_PRODUCTION`.
