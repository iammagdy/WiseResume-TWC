...
> **Whop Integration State — 2026-09-09:** Live manual QA subscription enrollment gate resolved and verified live (`can_subscribe = true`). Target manual QA account `debeg50114@fidhost.com` (`6aa1***ec13`) is consistently bound across all 3 hubs (`billing-checkout`, `ai-gateway`, `coupons`). `coupons` hub deployed to Appwrite via GitHub Actions (Run `34345158662`, deployment `6aa14f24...`, source hash `669bd361...`) with provider-aware availability logic. Whop Production remains DISABLED / NOT ACTIVATED. Public checkout remains strictly isolated to `WHOP_SANDBOX_QA_USER_ID`.

**Last Verified:** 2026-09-09
**Status:** `LIVE_MANUAL_QA_READY__SUBSCRIPTION_ENROLLMENT_OPEN` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `coupons`: `DEPLOYED_AND_VERIFIED_LIVE`, `billing-checkout`, `whop-webhook`, `ai-gateway`: `CURRENT_AND_READY`, Whop Schema: `READY`, Whop Sandbox Lifecycle: `VERIFIED_LIVE`, Whop Production: `DISABLED / NOT ACTIVATED`, Sandbox Checkout: `QA-GATED ONLY`)
