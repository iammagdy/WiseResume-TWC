...
**Status:** `WHATS_NEW_IMPLEMENTED_LOCAL_BROWSER_VERIFIED / VERCEL_PREVIEW_BLOCKED_EXTERNAL_ACCESS` (Audit Baseline: `FULL_FUNCTIONALITY_AUDIT_P1_CLOSEOUT_COMPLETE`, Billing: `BILLING_CHECKOUT_DISABLED`) — Redesigned customer-facing `/whats-new` and `/ar/whats-new` experiences implemented on branch `feat/whats-new-revamp`. Shipped history reconciled through 2026-09-03 (40 verified releases):
* **What's New Page Full Revamp & Shipped-History Reconciliation (2026-09-03):** `WHATS_NEW_IMPLEMENTED_LOCAL_BROWSER_VERIFIED / VERCEL_PREVIEW_BLOCKED_EXTERNAL_ACCESS` — Transformed `/whats-new` and `/ar/whats-new` from an un-scannable 9,427px chronological feed into a modern, responsive release hub with monthly progressive disclosure. Initial page scroll height materially reduced: Desktop EN 5,011px (vs old 9,427px / prior revamp 11,531px), Desktop AR 4,857px, Mobile EN 7,174px (vs prior 17,397px), Mobile AR 6,459px (vs prior 15,477px). Architecture: Latest Highlights (top 3 September releases: More Reliable Native PDF Exports, Asynchronous LinkedIn Profile Optimization, and Instant Tailoring Cancellation) with feed deduplication (September feed renders compact reference banner and non-featured updates in default view; filters automatically expose all matching cards). Monthly progressive disclosure keeps September and August 2026 expanded by default while older months (July 2026 through October 2025) are collapsed by default behind accessible semantic button controls (`aria-expanded`, `aria-controls`, `aria-label`, visible focus). Dual Type (`All` | `New` | `Improved` | `Fixed`) and Product Area (`All Areas` | `Resume & Portfolio` | `AI & Tailoring` | `Jobs & Career` | `Platform` | `Security & Legal`) filters, and Month Jump selectors automatically expand matching months. Strictly qualified all public customer copy against production evidence: removed unmeasured bandwidth/battery claims, eliminated "across all templates" / "serverless Chromium" PDF absolutes, qualified autosave to factual Appwrite read deduplication, tempered tailoring cancellation to client lifecycle clearing, and framed portfolio contact around visitor inquiries with Turnstile spam prevention. Assigned explicit `updateType: 'new' | 'improved' | 'fixed'` across all 40 releases (19 new, 16 improved, 5 fixed). Full test suite (7 files, 55 tests) and production build passed cleanly. Local browser QA verified across all 8 matrix combinations (Desktop 1440px & Mobile 390px x EN LTR & AR RTL x Light & Dark) with 0 console errors and 0px horizontal overflow. Remote Vercel Preview browser QA status: `BLOCKED_EXTERNAL_ACCESS` (Vercel Deployment Protection SSO login redirects unauthenticated probes to `https://vercel.com/sso-api?...`; owner authenticated browser session, share link, or bypass token required to view remote preview; production merge pending owner review).
