...
* **AI Studio LinkedIn Optimizer Async Remediation (2026-09-03):** `LINKEDIN_ASYNC_FIX_PR_READY / PRODUCTION_UNVERIFIED` — Scoped PR created on branch `fix/ai-studio-linkedin-async-execution`. Replaces synchronous HTTP execution (`async: false`, failing with HTTP 408 at ~15s) with background worker execution (`async: true`) + 1.5s client polling + generic idempotency cache result retrieval. 15/15 unit tests passing across LinkedIn and Tailoring suites. Zero changes to Appwrite backend deployment, database schema, or provider routing. Live production verification pending PR merge and deployment.
