...
**Status:** `PAYPAL_PHASE4_FINAL_CORRECTIVE_SAFETY_PASS_PR_READY` (PR: #287, Branch: `feat/paypal-sandbox-phase4`, Audit Baseline: `FULL_FUNCTIONALITY_AUDIT_P1_CLOSEOUT_COMPLETE`, Billing: `BILLING_CHECKOUT_DISABLED`) — PayPal Phase 4 final corrective safety, contract hardening, idempotency lifecycle, and UI verification pass complete on PR #287. All 20 review directives satisfied across backend hubs, deployment workflows, frontend libraries, UI workspaces, and 120 automated contract/regression tests (101 Node tests + 19 Vitest tests). Production checkout remains strictly disabled (`BILLING_CHECKOUT_DISABLED`).
* **PayPal Sandbox Phase 4 Final Corrective Safety & UI Verification Pass Complete (2026-09-04):** `PAYPAL_PHASE4_FINAL_CORRECTIVE_SAFETY_PASS_PR_READY` — Addressed all 20 owner review requirements on PR #287 (`feat/paypal-sandbox-phase4`): (1) P0 Sandbox QA Gate Fail-Closed: `assertRuntimeEnabled` strictly requires non-empty `config.qaUserId` AND `userId === config.qaUserId` in sandbox mode, returning HTTP 403 `payments_disabled` before any provider execution. (2) P0 Server Request ID Stable Derivation: `providerRequestId` deterministically derived as `wr_sub_${hash(session_key).slice(0, 32)}`. (3) P0 Ambiguous Provider Error Classification: `store.complete` persistence failure after PayPal 201 classifies as `uncertain` (`markUncertain`), never terminal `failed`. (4) Cancellation Server Derivation & Envelope Sanitization: Removed client `subscriptionId` dependency from `cancelBillingSubscription` and `service.cancel()`; resolves strictly by authenticated `userId` and strips `subscription_id` from the public response envelope. (5) Deploy Script & Workflow Catalog Wiring: Synced exact Sandbox catalog IDs (`P-3A193536YV1432359NKM36QY`, `P-17M39010JR353545NNKM36RA`, `PROD-8XE5253028560521H`); removed dead plan IDs/URLs; missing `BILLING_CHECKOUT_PROVIDER` strictly fails closed. (6) Environment-Specific Client Origin Defense: `getApprovedPayPalOrigins` returns only `https://www.sandbox.paypal.com` in sandbox, only `https://www.paypal.com` in production, and fails closed (`[]`) otherwise. (7) Frontend Attempt Key Idempotency: `getOrCreatePlanAttemptKey` and `clearPlanAttemptKey` in `sessionStorage`; reuses on retryable error; clears on non-retryable error, plan switch, cancel, or approval. (8) Two-Stage Cancellation UX & Copy Precision: POST accepted -> "Cancellation requested. Updating your subscription…" -> 30s polling -> confirmed; neutral copy when `provider_expires_at` is missing (stops future renewals, never claims "until the end of your billing period"). (9) Timeout Neutral Copy: 90s polling timeout displays "Taking Longer Than Usual", never claiming "Payment Received". (10) Fail-Closed `can_subscribe`: strictly defaults to `false` when missing/undefined. (11) Motion-Safe: added `motion-reduce:animate-none` on spinners. All 101 Node hub tests and 19 Vitest tests pass cleanly. Zero live transactions created.
