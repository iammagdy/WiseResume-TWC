...
**Status:** `WHATS_NEW_IMPLEMENTED_PREVIEW_VERIFIED / PRODUCTION_UNVERIFIED` (Audit Baseline: `FULL_FUNCTIONALITY_AUDIT_P1_CLOSEOUT_COMPLETE`, Billing: `BILLING_CHECKOUT_DISABLED`) — Redesigned customer-facing `/whats-new` and `/ar/whats-new` experiences implemented on branch `feat/whats-new-revamp`. Shipped history reconciled through 2026-09-03 (40 verified releases):
* **What's New Page Full Revamp & Shipped-History Reconciliation (2026-09-03):** `WHATS_NEW_IMPLEMENTED_PREVIEW_VERIFIED / PRODUCTION_UNVERIFIED` — Transformed `/whats-new` and `/ar/whats-new` from an un-scannable 9,427px chronological feed into a modern, responsive release hub. Features: Latest Highlights (top 3 September releases: Native PDF Export, Asynchronous LinkedIn Optimizer, and Instant Tailoring Cancellation), dual Type (`All` | `New` | `Improved` | `Fixed`) and Category filters, Month Jump selectors, clean monthly section dividers, and progressive disclosure for the older 2025 archive. Reconciled 6 customer-facing September 2026 releases (PRs #263, #265, #267, #271, #276, #278) bringing the verified dataset from 34 to 40 items. Full test suite (7 files, 47 tests) and production build passed cleanly. Real browser QA verified across all 8 matrix combinations (Desktop 1440px & Mobile 390px x EN LTR & AR RTL x Light & Dark) with 0 console errors and 0px horizontal overflow.
