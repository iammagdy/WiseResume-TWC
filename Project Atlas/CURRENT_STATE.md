...
* **Coupon Live Schema Compatibility Fix (2026-09-07):** `COUPON_LIVE_SCHEMA_COMPAT_READY_FOR_REVIEW` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `paypal-webhook`: `DEPLOYED_SANDBOX` [run 34110889444], `coupons`: `PENDING_TARGETED_DEPLOYMENT`, PayPal Schema: `READY`, Coupon Schema Compat: `IN_REVIEW`, `PAYPAL_PRODUCTION_READY = NO`, Base Main SHA: `ac8172fa226d5eadc29163b54edf5511463e6575`) — Target deployment attempt for `coupons` (run `34113008193`) failed during `Ensure coupon security schema` with `Incompatible attribute "discount_codes.code": size 50 (expected 64)`. Implemented narrow non-destructive legacy tolerance in `scripts/setup_discount_codes_schema.cjs` for proven live Appwrite collection states: (1) `discount_codes.code` accepts string size 50 or 64 (`required=true`); (2) `coupon_redemptions.user_id` accepts optional status (`required=false` or `true`) and size >= 64 (preserving live size 65000). All other attributes and all indexes remain strictly fail-closed. Expanded `tests/hubs/coupon-schema.test.cjs` with 12 contract assertions. All 16 coupon schema and 384 hub tests pass. Zero business logic changed; source hashes untouched; Production PayPal untouched.
* **Coupon Schema Pre-Deployment Readiness Hardening (Historical) (2026-09-07):** `COUPONS_SCHEMA_PREDEPLOY_HARDENING_READY_FOR_REVIEW` (Frontend: `DEPLOYED_TO_PRODUCTION` via Vercel, Backend: `paypal-webhook`: `DEPLOYED_SANDBOX` [run 34110889444], `coupons`: `PENDING_TARGETED_DEPLOYMENT`, PayPal Schema: `READY`, Coupon Schema Hardening: `MERGED` [PR #306 at `ac8172fa`], `PAYPAL_PRODUCTION_READY = NO`, Base Main SHA: `2341c263a1570f94dbafd80e9e3c901b87d366eb`) — Hardened coupon schema provisioner in `scripts/setup_discount_codes_schema.cjs` prior to targeted deployment of the `coupons` hub: (1) Added pre-mutation identifier preflight validation (`^[A-Za-z][A-Za-z0-9._-]{0,35}) preventing malformed or oversized schema identifiers from reaching Appwrite; (2) Added index readiness polling (`waitForIndexAvailable`) guaranteeing both `code_unique` and `discount_code_idx` are in `available` status before schema completion; (3) Added non-destructive index compatibility checks that fail closed on type/attribute discrepancies without dropping indexes; (4) Added attribute compatibility checks with documented legacy tolerance for `coupon_redemptions.user_id`; (5) Server-only collection permissions and documentSecurity=false verified. Expanded test suite in `tests/hubs/coupon-schema.test.cjs` from 3 to 16 tests covering items A through M. All 384 hub tests passing (100%). Zero coupons business logic changed; source hashes unchanged; Production PayPal untouched.
