...
**Status:** `PAYPAL_STAGE_A_RECONCILED_DOMAIN_RECOVERY_PENDING_STAGE_B` (Branch: `fix/paypal-stage-a-reconciliation`, Target: `main`, Audit Baseline: `FULL_FUNCTIONALITY_AUDIT_P1_CLOSEOUT_COMPLETE`, Billing: `BILLING_CHECKOUT_DISABLED`) — PayPal Stage A deployment and schema applied on live Appwrite cluster via workflow runs 33917547251 & 33918098250; diagnostic direct-main commit reverted cleanly on recovery branch; runtime preflight and domain probe completed: (1) Stage A paypal-webhook function DEPLOYED in bootstrap mode (smoke check HTTP 400 fail-closed verified). (2) Schema APPLIED / VERIFIED on live cluster (paypal_subscription_state and paypal_event_ledger provisioned server-only). (3) Secrets PROVISIONED (PAYPAL_SANDBOX_CLIENT_ID, PAYPAL_SANDBOX_CLIENT_SECRET, BILLING_CHECKOUT_QA_USER_ID configured). (4) PayPal MCP HEALTHY (Product PROD-8XE5253028560521H, Pro P-3A193536YV1432359NKM36QY $5/mo, Ultimate P-17M39010JR353545NNKM36RA $10/mo verified ACTIVE). (5) Webhook registration NOT PERFORMED (0 webhooks in Sandbox; candidate https://paypal-webhook.fra.appwrite.run probed returning HTTP 404 router rule not found; exact generated domain retrieval requires Appwrite Console OWNER_ACTION_REQUIRED_FUNCTION_DOMAIN). (6) Stage B PENDING; consumers (billing-checkout, coupons, ai-gateway) NOT DEPLOYED; real signed webhook NOT VERIFIED; E2E NOT RUN; non-QA runtime checkout safety NOT_YET_VERIFIED. (7) Pro credit policy: 50 AI credits/day, Ultimate unlimited. (8) Production PayPal UNTOUCHED; public checkout DISABLED (`BILLING_CHECKOUT_ENABLED=false`); plan revision DEFERRED (`PAYPAL_PLAN_CHANGE_REVISION_OWNER_DECISION_REQUIRED`). (9) What's New decision: `WHATS_NEW_DEFER_UNTIL_PRODUCTION`.
* **PayPal Sandbox Stage A Deployed & Domain Recovery Pass (2026-09-04):** `PAYPAL_STAGE_A_RECONCILED_DOMAIN_RECOVERY_PENDING_STAGE_B` — Branch `fix/paypal-stage-a-reconciliation` (Target: `main`): (1) Stage A deployment completed successfully on live Appwrite cluster via GitHub Actions workflow `deploy-appwrite-hubs.yml` (runs `33917547251` and `33918098250`). Schema `paypal_subscription_state` and `paypal_event_ledger` applied and verified server-only. Fail-closed smoke check confirmed HTTP 400. (2) Accidental direct-main diagnostic logging in `scripts/deploy_hubs.cjs` reverted cleanly on recovery branch restoring content from clean post-merge SHA `174af382c399ba4d72e37dbb0bb9d4d6d6112c38`. (3) GitHub repository secrets `PAYPAL_SANDBOX_CLIENT_ID`, `PAYPAL_SANDBOX_CLIENT_SECRET`, and `BILLING_CHECKOUT_QA_USER_ID` provisioned securely; non-secret variables `BILLING_CHECKOUT_PROVIDER=paypal`, `BILLING_CHECKOUT_ENABLED=false`, `BILLING_CHECKOUT_PROVIDER_READY=false` configured. (4) PayPal MCP healthy and verified active (Product `PROD-8XE5253028560521H`, Pro `P-3A193536YV1432359NKM36QY`, Ultimate `P-17M39010JR353545NNKM36RA`). (5) Webhook audit confirmed 0 webhooks in Sandbox; candidate `https://paypal-webhook.fra.appwrite.run` probed (DNS/TLS pass, HTTP 404 router rule not found); exact domain retrieval gated on Appwrite Console (`OWNER_ACTION_REQUIRED_FUNCTION_DOMAIN`). Stage B, consumers (`billing-checkout`, `coupons`, `ai-gateway`), and live E2E remain pending. Pro AI policy: 50 credits/day; Ultimate unlimited. Production PayPal untouched; public checkout disabled; plan revision deferred. What's New decision: `WHATS_NEW_DEFER_UNTIL_PRODUCTION`.
