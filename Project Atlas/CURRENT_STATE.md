...
**Status:** `PDF_EXPORT_ESM_IMPORT_REMEDIATED_LOCAL_VERIFIED / AWAITING_PRODUCTION_DEPLOYMENT` with `BILLING_CHECKOUT_DISABLED` — Production snapshot updated following Native PDF Export ESM relative import repair:
* **Native PDF Export P1 ESM Relative Import Remediation (2026-09-03):** `PDF_EXPORT_ESM_IMPORT_REMEDIATED_LOCAL_VERIFIED` — PR #275 deployed successfully (`dpl_BF39bwHg4dFnqB3zpcNyddvtYgrx`), resolving Chromium binary packaging and lazy import structure. Production execution then produced the definitive container runtime exception: `Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/var/task/api/_lib/appwriteDocumentId' imported from /var/task/api/export/pdf-native.js`. Root cause confirmed as `ROOT_CAUSE_CONFIRMED_ESM_RELATIVE_IMPORT`: Node ESM emitted for Vercel serverless functions requires explicit `.js` extensions for relative imports. Remediation applied: updated import in `api/export/pdf-native.ts` to `import { createAppwriteDocumentId } from '../_lib/appwriteDocumentId.js';` and strengthened test in `src/lib/security/pdfNativeRuntimeImports.test.ts`. Verified locally: 0 TypeScript errors, 9/9 passing Vitest export tests, clean production build in 52.22s, and successful unbundled Node 22 native ESM import evaluation. Ready for PR, merge, Vercel deployment, and live production browser QA.
