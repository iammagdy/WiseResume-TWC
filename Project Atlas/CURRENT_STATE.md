...
* **PayPal Cancellation Paid-Through Fix & Runtime E2E Re-Verification (2026-09-06):** `PAYPAL_CANCELLATION_FIX_RUNTIME_VERIFIED` — Branch `docs/paypal-cancellation-fix-runtime-verification` (Target: `main`): PR #294 merged to preserve paid access through the PayPal cancellation period; PR #295, #296, #297 merged to harden webhook activation supersession, ledger reclaim of recoverable ignored events, and initial payment timestamp handling. Runtime E2E verified in live US PayPal Sandbox with QA user `qa_pp_afbf725e` and subscription `I-58K84FGAFFHL`. Customer UI cancellation performed on `https://wiseresume.app/subscription`. Direct PayPal REST API confirmed `CANCELLED` at `2026-09-06T10:59:18Z`. First automatic PayPal webhook `WH-9AA0732263469183V-35B12222V7015642N` processed cleanly. Appwrite `paypal_subscription_state` and coupons resolver verify `status = canceled`, `expires_at = 2026-10-06T10:00:00.000Z` (PRESERVED), `effective_plan = premium` (unlimited AI quota retained until Oct 6, 2026). Live UI verified: "Ultimate [Canceled]" with notice "Access ends on October 6, 2026" and cancel button removed. Public checkout fail-closed restored (`BILLING_CHECKOUT_ENABLED=false`, `BILLING_CHECKOUT_PROVIDER_READY=false`) and deployed; HTTP 403 `payments_disabled` verified. All 180 backend and 28 frontend unit/integration tests pass.
