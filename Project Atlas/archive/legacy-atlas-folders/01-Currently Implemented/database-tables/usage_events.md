...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.usage_events.Row` (5 columns)
...
**What it is:** Frontend analytics events (page_view, feature_use, etc.).

**Schema:** 5 columns per `Tables.usage_events.Row` in `src/integrations/supabase/types.ts`. Columns: `created_at`, `event_type`, `id`, `metadata`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.usage_events.Row` in `types.ts`).
