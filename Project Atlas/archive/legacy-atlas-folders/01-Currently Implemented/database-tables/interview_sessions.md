...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.interview_sessions.Row` (12 columns)
...
**What it is:** AI mock interview sessions.

**Schema:** 12 columns per `Tables.interview_sessions.Row` in `src/integrations/supabase/types.ts`. Columns: `created_at`, `duration_seconds`, `id`, `improvements`, `interview_type`, `job_description`, `job_title`, `messages`, `overall_score`, `resume_id`, `strengths`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.interview_sessions.Row` in `types.ts`).
