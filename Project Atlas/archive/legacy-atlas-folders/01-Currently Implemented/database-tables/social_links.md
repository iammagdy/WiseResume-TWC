...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.social_links.Row` (6 columns)
...
**What it is:** Per-user social profile links.

**Schema:** 6 columns per `Tables.social_links.Row` in `src/integrations/supabase/types.ts`. Columns: `created_at`, `id`, `platform_key`, `updated_at`, `url`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.social_links.Row` in `types.ts`).
