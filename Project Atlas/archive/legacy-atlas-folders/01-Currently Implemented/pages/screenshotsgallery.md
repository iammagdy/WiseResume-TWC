# ScreenshotsGalleryPage

  **Last verified:** 2026-04-17
  **Type:** reference card
  **Sources:**
  - `src/pages/ScreenshotsGalleryPage.tsx`
  - `src/AppInterior.tsx` (route registration)
  - `project-governance/PRODUCT.md` §2


  **Canonical owner:** `project-governance/PRODUCT.md` §2

  ---

  **What it is:** Generated app store screenshots gallery.

  **Route(s):**
  - `  /screenshots-gallery`

  **Where it lives:** `src/pages/ScreenshotsGalleryPage.tsx`

  **Related:**
  - Pages index: `Project Atlas/01-Currently Implemented/pages/README.md`
  