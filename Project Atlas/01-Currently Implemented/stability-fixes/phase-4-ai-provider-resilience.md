...
- Admin observability: read-only `ai-breaker-status` edge function exposes the current breaker state per provider (one row per provider with `failure_count`, `window_started_at`, `opened_until`, `last_success_at`, `last_failure_at`, derived `is_open`). Gated by `requireAdminAuth`. → `supabase/functions/ai-breaker-status/index.ts`, `supabase/functions/_shared/adminAuth.ts`. **Removed 2026-04-24 (Task #21):** the function was never wired into the AI Provider panel (the panel uses `/api/admin/ai-provider/openrouter-status` etc.) and had no source on disk; it was deleted from the deployment. Re-implement from this spec if a real admin breaker view is built.
