...
The Wise Cloud is a single React + Vite SPA backed by Appwrite Cloud. It runs two distinct products on the same infrastructure.
...
| Database | Appwrite Databases | `src/lib/appwrite-collections.ts` |
| Backend | Supabase Edge Functions (Deno) — 93 functions | `supabase/functions/` |
| AI | Appwrite `ai-gateway` routed provider chain | `appwrite-hubs/ai-gateway/src/main.js` |
...
| Hosting | Hostinger (frontend) + Appwrite Cloud (backend) | `.github/workflows/deploy-frontend.yml`, `.github/workflows/deploy-appwrite-hubs.yml` |
...
| Functions | Appwrite hubs + routed features | `appwrite-hubs/`, `Project Atlas/01-Currently Implemented/functions/` |
