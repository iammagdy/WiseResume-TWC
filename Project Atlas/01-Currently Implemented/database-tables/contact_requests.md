...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.contact_requests.Row` (9 columns)
...
**What it is:** Outbound contact requests.

**Schema:** 9 columns per `Tables.contact_requests.Row` in `src/integrations/supabase/types.ts`. Columns: `created_at`, `email`, `id`, `ip_address`, `message`, `metadata`, `subject`, `type`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.contact_requests.Row` in `types.ts`).
