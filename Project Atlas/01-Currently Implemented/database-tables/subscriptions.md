...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.subscriptions.Row` (12 columns)
...
**What it is:** Stripe subscription rows. RLS hardening 2026-04-17 → clients SELECT-only.

**Schema:** 12 columns per `Tables.subscriptions.Row` in `src/integrations/supabase/types.ts`. Columns: `ai_credits_monthly`, `ai_credits_topup`, `created_at`, `current_period_end`, `current_period_start`, `id`, `plan_name`, `status`, `stripe_customer_id`, `stripe_subscription_id`, `updated_at`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.subscriptions.Row` in `types.ts`).
