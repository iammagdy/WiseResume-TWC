...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.resume_shares.Row` (9 columns)
...
**What it is:** Per-resume share-link records.

**Schema:** 9 columns per `Tables.resume_shares.Row` in `src/integrations/supabase/types.ts`. Columns: `created_at`, `expires_at`, `id`, `is_active`, `password`, `resume_id`, `token`, `user_id`, `view_count`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.resume_shares.Row` in `types.ts`).
