...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.push_subscriptions.Row` (6 columns)
...
**What it is:** Web push notification subscriptions.

**Schema:** 6 columns per `Tables.push_subscriptions.Row` in `src/integrations/supabase/types.ts`. Columns: `auth`, `created_at`, `endpoint`, `id`, `p256dh`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.push_subscriptions.Row` in `types.ts`).
