...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.feature_requests.Row` (10 columns)
...
**What it is:** In-app feature request submissions.

**Schema:** 10 columns per `Tables.feature_requests.Row` in `src/integrations/supabase/types.ts`. Columns: `app_version`, `created_at`, `feature_description`, `feature_title`, `id`, `route`, `status`, `user_agent`, `user_email`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.feature_requests.Row` in `types.ts`).
