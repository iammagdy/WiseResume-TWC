...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.short_links.Row` (7 columns)
...
**What it is:** Short-link entries served by `/l/:linkId`.

**Schema:** 7 columns per `Tables.short_links.Row` in `src/integrations/supabase/types.ts`. Columns: `click_count`, `created_at`, `id`, `label`, `owner_user_id`, `portfolio_username`, `target_url`.
