...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.messages.Row` (13 columns)
...
**What it is:** Direct messages. Soft-delete via `is_deleted`.

**Schema:** 13 columns per `Tables.messages.Row` in `src/integrations/supabase/types.ts`. Columns: `content`, `created_at`, `deleted_at`, `email`, `full_name`, `id`, `is_deleted`, `metadata`, `status`, `subject`, `type`, `updated_at`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.messages.Row` in `types.ts`).
