...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.user_api_keys.Row` (9 columns)
...
**What it is:** BYOK encrypted keys (AES-GCM-256, per-user salt).

**Schema:** 9 columns per `Tables.user_api_keys.Row` in `src/integrations/supabase/types.ts`. Columns: `base_url`, `created_at`, `encrypted_key`, `id`, `key_tier`, `model`, `provider`, `updated_at`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.user_api_keys.Row` in `types.ts`).
