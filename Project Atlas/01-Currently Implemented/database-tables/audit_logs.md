...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.audit_logs.Row` (6 columns)
...
**What it is:** Generic audit trail.

**Schema:** 6 columns per `Tables.audit_logs.Row` in `src/integrations/supabase/types.ts`. Columns: `action`, `category`, `created_at`, `id`, `metadata`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.audit_logs.Row` in `types.ts`).
