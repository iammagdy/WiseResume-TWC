...
**Type:** reference card
**Sources:**
- `src/integrations/supabase/types.ts` — `Tables.notifications.Row` (8 columns)
...
**What it is:** In-app notifications inbox.

**Schema:** 8 columns per `Tables.notifications.Row` in `src/integrations/supabase/types.ts`. Columns: `created_at`, `id`, `is_read`, `link`, `message`, `title`, `type`, `user_id`.

**Owner FK pattern:** `user_id` → `auth.users.id` (per `Tables.notifications.Row` in `types.ts`).
