...
**Last verified:** 2026-05-13 (Atlas section renamed to `functions/` and reconciled against Appwrite-native architecture)
**Type:** index
**Sources:**
- `appwrite-hubs/` and the current Atlas function cards
- `Project Atlas/MASTER_HANDOVER_2026.md`
- `Project Atlas/GOVERNANCE.md`

**Canonical owner:** `appwrite-hubs/` and the live Appwrite Function inventory.
...
Goal: stay below the current platform's function deployment envelope.

## Maintenance — adding a new function

1. Create the new Appwrite Function source under `appwrite-hubs/<name>/` or document the routed feature inside `ai-gateway` when it is not a standalone function.
2. Document its auth model and execution path.
3. Wire the relevant Appwrite permissions, variables, and in-function guards.
4. Add or update the Atlas reference card here.
5. Update this index.
