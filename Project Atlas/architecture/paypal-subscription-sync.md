...
8. **Legacy Migration-on-Touch & Provider Sale Fallback Correlation:**
   ...
   - **Step 3 Provider Sale Fallback Correlation:** Authentic PayPal `PAYMENT.SALE.REFUNDED` webhook resources contain `sale_id` but omit `billing_agreement_id`. When correlation via payload `billing_agreement_id`, current provider state `last_entitlement_payment_id`, and historical sale ledger cannot resolve the subscription, `fetchSaleDetails(event.paymentId)` queries `GET /v1/payments/sale/{paymentId}` at PayPal provider. It validates `sale.id === paymentId` and `sale.billing_agreement_id.startsWith('I-')`, then assigns `event.subscriptionId` and loads previous state. If `event.customId` and `sale.custom` conflict, or if trusted custom conflicts with `previous.user_id`, it fails closed (`correlation_identity_conflict`).
   - **Narrowed Rejected Event Reclaim:** The transactional reclaim check (`isReclaimableRejectedCorrelation`) permits redelivery recovery for events that were rejected under `outcome_code === 'unresolved_subscription_correlation'` on refund/reversal events with `paymentId`, allowing PayPal redeliveries to recover previously rejected authentic refund events safely.
   - If the canonical subscription ID cannot be resolved, provider `start_time` is missing/invalid, or `tx.time` is malformed, it fails closed with zero state mutation (`unresolved_subscription_correlation`, `unresolved_legacy_payment_correlation`, or retryable 502 `invalid_provider_transaction_time`). No bulk backfill is required.
