...
3. **Historical Refund and Reversal Policy:**
   - When a refund or reversal event arrives for a `payment_id` that is not the subscription's `last_entitlement_payment_id`:
     - **Authoritative payment timestamps:** Ordering comparison strictly relies on state (`last_entitlement_payment_timestamp_ms`) or ledger-recorded historical payment timestamps (`payment_timestamp_ms`). Provider transaction string timestamps (`tx.time`) are NEVER used for the ordering comparison (preventing malformed date strings from producing `NaN` and misclassifying historical refunds as current).
