...
## RevenueCat provider-state collections (Phase 2A applied and verified)
...
Both collections have live `permissions=[]` and `documentSecurity=false`; browsers have no direct read or write access. The live `user_id_unique` and `event_id_unique` indexes are available and enforce the intended uniqueness contracts. `scripts/setup_revenuecat_schema.cjs` remains idempotent and fail-closed. The existing overloaded `subscriptions` attributes, permissions, and documents were not modified. The RevenueCat Function, secret, webhook configuration, and payment activation remain outside this phase.
