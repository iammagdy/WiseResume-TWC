...
* **Production delivery prerequisite (2026-08-11):** the deployed verification function has no configured Resend API key or sender identity while the Appwrite verification template is blanked for the branded route. Actual verification delivery must remain unverified until the owner configures those server-side values; no client-side bypass or manual verification is permitted.
