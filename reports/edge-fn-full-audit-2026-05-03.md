...
> **RESOLVED 2026-05-03 (Task #67, Phase 3).** Decision: **keep 200 with clearer payload.** The empty-candidates branch is reached when the resume editor calls `smart-fit-rewrite` after the user has already filtered every suggestion out via the accept/reject UI — surfacing a 400 there would be a UX regression for input the user did not author. Updated the response envelope to `{success:true, outcomes:[], reason:"no-op-empty-input"}` so callers can distinguish a genuine no-op from a successful rewrite. Documented the intent in two places: a top-of-file `/** */` docstring on `smart-fit-rewrite/index.ts` (under "Empty-input intent") AND a multi-line in-branch comment at the no-op site itself. **Scope clarification:** the no-op branch deliberately swallows any zero-work request — a missing/null `candidates` field (handled by the `?? []` fallback) and an explicit `candidates: []` both return the same 200 envelope. A non-array truthy value (e.g. a string or object) is NOT coerced by `??`; it would throw at the `.slice(...)` call and fall out to the outer try/catch as a 500. That is acceptable because the orchestrator never sends malformed `candidates` in production, and treating genuine zero-work cases uniformly is better UX than a 400. Real per-candidate schema validation still happens in the AI loop below for non-empty inputs and surfaces through the standard `toUserError` envelope. File: `supabase/functions/smart-fit-rewrite/index.ts` (line ~298). The separate `noauth→500` issue still tracked under §7 YELLOW is a distinct H1-class problem (smart-fit-rewrite was excluded from the H1 batch redeploy because it owns its own auth wrapper); fixing it is a Phase 4 item.
...
> **RESOLVED 2026-05-03 (Task #67, Phase 3).** Dropped the explicit `status: 204` from the OPTIONS branch in `wise-ai-chat/index.ts` so the response defaults to 200, matching the pattern in every other edge function (`return new Response(null, { headers: corsHeaders })`). In-source comment notes the parity rationale and the resolution tag. No behavioural risk — both 200 and 204 are valid CORS preflight success codes per RFC 7231 §6.3 and every browser treats them identically. Redeployed via `npx supabase functions deploy wise-ai-chat --project-ref jnsfmkzgxsviuthaqlyy`.
...
> **RESOLVED 2026-05-03 (Task #67, Phase 3).** Added 8–14 line `/** */` headers covering **purpose / trigger / auth posture / dispatch contract** to the 13 admin functions that had no leading documentation: `admin-audit-logs`, `admin-delete-user`, `admin-devkit-data`, `admin-email`, `admin-get-identity`, `admin-impersonate`, `admin-list-user-content`, `admin-list-users`, `admin-merge-identity`, `admin-moderation`, `admin-onboarding-funnel`, `admin-portfolio-usernames`, `admin-save-note`. Also upgraded `admin-check-access` from a 4-line `//` header (purpose only) to a full `/** */` block. Per code-review feedback (round 3), also converted the four router admin functions — `admin-ai-ops`, `admin-config`, `admin-user-ops`, `admin-wisehire` — from their existing 26–46-line `//`-style headers to `/** */` JSDoc blocks (content preserved verbatim, comment-syntax change only). `admin-kinde-reconcile`, `admin-owner-ops`, and `hard-purge` already had `/** */` blocks. Net result: every admin function now has discoverable purpose/trigger/auth/dispatch documentation in a leading `/** */` block. Pure operability change — zero runtime impact.
