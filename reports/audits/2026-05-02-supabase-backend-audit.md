...
**Status (Task #25, 2026-05-02):** **CLOSED.** `WISE_ENV=production` was set on project `jnsfmkzgxsviuthaqlyy` via the Supabase Management API (`POST /v1/projects/{ref}/secrets`, HTTP 201). Verified present in the secret list afterwards. `admin-devkit-data` now reads `WISE_ENV` first on every Mission Control request — the legacy `DENO_DEPLOYMENT_ID` heuristic is no longer in the hot path on production. No follow-up needed for prod; non-prod projects can be set to `WISE_ENV=dev` whenever they're spun up.
