...
export default function CoverLetterNewPage() {
  ...
  const handleGenerate = async (force = false) => {
    ...
    try {
      ...
      if (!letter || !letter.trim()) {
        throw new Error('AI returned empty cover letter text.');
      }
      ...
      if (returnTo) {
        if (coverLetterId && selectedResumeId) {
          try {
            ...
            console.error('[CoverLetter] Navigation failed:', navErr);
            toast.error('Failed to return to the tailoring result automatically. You can copy, download, or save it below.');
            setShowResultAnyway(true);
          }
        } else {
          console.warn('[CoverLetter] coverLetterId or selectedResumeId is missing, skipping auto-return.');
          toast.warning('Cover letter generated but could not be linked automatically. You can save, download, or copy it below.');
          setShowResultAnyway(true);
        }
      } else {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const doc = await saveCoverLetter.mutateAsync({
        title: jobTitle ? `${jobTitle}${company ? ` - ${company}` : ''}` : 'Cover Letter',
      ...
      navigate(`/cover-letter/edit/${doc.id}`, { replace: true });
    } catch (err) {
      console.error('[CoverLetter] Save failed:', err);
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto overscroll-y-contain">
        <div className="px-4 pt-4 pb-32 space-y-5">
          ...
          {result && (!returnTo || showResultAnyway) && (
            <motion.div
      ...
      {result && (!returnTo || showResultAnyway) && (
        <div className="fixed bottom-[calc(4rem+env(safe-area-inset-bottom))] lg:bottom-0 left-0 right-0 z-40 px-4 lg:px-6 lg:pb-4">
          <div className="bg-background/95 backdrop-blur-sm border border-border rounded-2xl p-3 flex gap-2 shadow-lg">
            ...
            <Button size="sm" className="gap-1.5 flex-1" onClick={handleSave} disabled={!result.trim() || isSaving}>
              {isSaving ? <MiniSpinner size={16} /> : <Save className="w-4 h-4" />} {isSaving ? 'Saving...' : 'Save'}
            </Button>
          </div>
        </div>
      )}
