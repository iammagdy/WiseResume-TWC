...
/** A single fluffy auth cloud */
function AuthCloud({
  top, scale, duration, delay, opacity,
}: {
  top: number; scale: number; duration: number; delay: number; opacity: number;
}) {
  return (
    <div
      aria-hidden="true"
      style={{
        ...
        animation: prefersReducedMotion
          ? 'none'
          : `auth-cloud-drift ${duration}s linear ${delay}s infinite`,
        willChange: 'transform',
      }}
    >
      <div style={{ position: 'absolute', bottom: 0, left: '16px', right: '16px', height: '32px', borderRadius: '999px', background: 'white' }} />
      <div style={{ position: 'absolute', bottom: '18px', left: '45px', width: '80px', height: '62px', borderRadius: '50%', background: 'white' }} />
      <div style={{ position: 'absolute', bottom: '15px', left: '18px', width: '52px', height: '44px', borderRadius: '50%', background: 'white' }} />
      <div style={{ position: 'absolute', bottom: '15px', left: '112px', width: '48px', height: '40px', borderRadius: '50%', background: 'white' }} />
    </div>
  );
}
...
function AuthBackground() {
  ...
  const lightClouds = useMemo(() => [
    { top: 8,  scale: 1.1,  duration: 62, delay: -10, opacity: 0.82 },
    { top: 26, scale: 0.75, duration: 48, delay: -35, opacity: 0.72 },
    { top: 50, scale: 0.92, duration: 74, delay: -55, opacity: 0.76 },
    { top: 70, scale: 0.65, duration: 56, delay: -22, opacity: 0.66 },
  ], []);
    ...
    <>
      <div
        ...
        style={{
          background: isDark
            ...
            : 'linear-gradient(160deg, hsl(200 85% 60%) 0%, hsl(207 78% 70%) 50%, hsl(215 65% 80%) 100%)',
      ...
      >
        {isDark ? (
            ...
            {starsRef.current.map((star) => (
              <div
                ...
                style={{
                  ...
                  animation: prefersReducedMotion ? 'none' : `auth-twinkle ${star.duration}s ease-in-out ${star.delay}s infinite`,
                }}
              />
            ))}
          </>
          ...
          <>
            {/* Sun glow top-right */}
            <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 55% 45% at 85% 0%, hsl(45 100% 88% / 0.72) 0%, transparent 55%)' }} />
            {/* Horizon haze */}
            <div style={{ position: 'absolute', inset: '0', bottom: 0, top: 'auto', height: '120px', background: 'linear-gradient(to top, hsl(220 60% 88% / 0.45) 0%, transparent 100%)' }} />
            {/* Clouds */}
            {lightClouds.map((c, i) => (
              <AuthCloud key={i} {...c} />
            ))}
