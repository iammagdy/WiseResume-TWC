...
function getCardProps(style: string): { className: string; style: React.CSSProperties } {
  switch (style) {
    case 'bold-dark':
      return { className: 'space-y-4 relative', style: { background: 'rgba(255,255,255,0.03)', border: '1px solid color-mix(in srgb, var(--pf-accent) 30%, transparent)', borderRadius: '1rem', padding: '1.5rem' } };
    case 'glass-pro':
      return { className: 'space-y-4 relative', style: { background: 'rgba(255,255,255,0.06)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '1rem', padding: '1.5rem' } };
    case 'classic-clean':
      return { className: 'space-y-4 relative', style: { borderLeft: '3px solid var(--pf-accent)', paddingLeft: '1.5rem', paddingTop: '1rem', paddingBottom: '1rem', borderRadius: '0 0.75rem 0.75rem 0', background: 'var(--pf-card, #f9f9f9)' } };
    case 'developer-terminal':
      return { className: 'pf-terminal-card space-y-4 relative', style: {} };
    case 'creative-spotlight':
      return { className: 'pf-spotlight-card space-y-4 relative', style: {} };
    case 'executive-suite':
      return { className: 'pf-executive-card space-y-4 relative', style: {} };
    case 'freelancer-starter':
      return { className: 'pf-starter-card space-y-4 relative', style: {} };
    case 'neon-cyber':
      return { className: 'pf-neon-card space-y-4 relative', style: {} };
    default:
      return { className: 'space-y-4 relative', style: { background: 'var(--pf-card, rgba(255,255,255,0.04))', border: '1px solid var(--pf-border, rgba(255,255,255,0.08))', borderRadius: '1rem', padding: '1.5rem' } };
  }
}
...
export function CaseStudyCard({ cs, style }: { cs: CaseStudy; style: string }) {
  ...
  const content = (
    <>
      ...
      <div className={isTerminal ? 'pf-terminal-card-body space-y-4' : ''}>
  ...
  return (
    <div className="pf-card-tilt">
      <motion.div ref={tiltRef} onPointerMove={onPointerMove} onPointerLeave={onPointerLeave} variants={unfold} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} style={cardProps.style} className={cardProps.className}>
        {content}
      </motion.div>
    </div>
  );
