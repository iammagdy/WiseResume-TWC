        ...
        <div className="space-y-3">
          {publications.map((pub, i) => (
            <div key={pub.id} className="rounded-xl border border-border overflow-hidden">
              <button onClick={() => setExpandedId(expandedId === pub.id ? null : pub.id)} className="w-full p-4 flex items-center justify-between hover:bg-muted touch-manipulation active:bg-muted/70 min-h-[72px]">
                <DragHandle />
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={e => { e.stopPropagation(); moveDown(i); }} disabled={i === publications.length - 1} className="p-1.5 rounded hover:bg-muted disabled:opacity-30 min-w-[40px] min-h-[40px] flex items-center justify-center touch-manipulation" aria-label="Move down"><ArrowDown className="w-3.5 h-3.5" /></button>
                </div>
              ...
              {expandedId === pub.id && (
                <div className="animate-in fade-in-0 duration-200"><div className="p-4 pt-0 space-y-4 border-t border-border">
                  <div><Label htmlFor={`pub-${pub.id}-title`} className="text-sm flex items-center gap-1.5 mb-2"><BookOpen className="w-4 h-4" />Title</Label><Input id={`pub-${pub.id}-title`} value={pub.title} onChange={e => updatePub(pub.id, { title: e.target.value })} placeholder="Publication Title" className="h-12" /></div>
                  <div><Label htmlFor={`pub-${pub.id}-publisher`} className="text-sm mb-2">Publisher / Journal</Label><Input id={`pub-${pub.id}-publisher`} value={pub.publisher} onChange={e => updatePub(pub.id, { publisher: e.target.value })} placeholder="Journal or Conference" className="h-12" /></div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><Label htmlFor={`pub-${pub.id}-date`} className="text-sm flex items-center gap-1.5 mb-2"><Calendar className="w-4 h-4" />Date</Label><Input id={`pub-${pub.id}-date`} value={pub.date} onChange={e => updatePub(pub.id, { date: e.target.value })} placeholder="2024" className="h-12" /></div>
                    <div><Label htmlFor={`pub-${pub.id}-coauthors`} className="text-sm mb-2">Co-Authors</Label><Input id={`pub-${pub.id}-coauthors`} value={pub.coAuthors || ''} onChange={e => updatePub(pub.id, { coAuthors: e.target.value })} placeholder="John Doe, Jane Smith" className="h-12" /></div>
                  </div>
                  <div><Label htmlFor={`pub-${pub.id}-url`} className="text-sm flex items-center gap-1.5 mb-2"><Link className="w-4 h-4" />URL / DOI</Label><Input id={`pub-${pub.id}-url`} value={pub.url || ''} onChange={e => updatePub(pub.id, { url: e.target.value })} placeholder="https://doi.org/..." className="h-12" type="url" /></div>
