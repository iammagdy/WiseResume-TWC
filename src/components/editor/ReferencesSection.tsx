...
import { Plus, Trash2, ChevronDown, ChevronUp, Users, Mail, Phone, Building2 } from 'lucide-react';
          ...
          {references.map((ref, i) => (
            <div key={ref.id} className="rounded-xl border border-border overflow-hidden">
              <button onClick={() => setExpandedId(expandedId === ref.id ? null : ref.id)} className="w-full p-4 flex items-center justify-between hover:bg-muted/50 touch-manipulation active:bg-muted/70 min-h-[72px]">
                <div className="text-left flex-1 min-w-0 pr-3"><p className="font-semibold text-sm truncate">{ref.name || `Reference ${i + 1}`}</p><p className="text-sm text-muted-foreground truncate">{ref.title ? `${ref.title} at ${ref.company}` : 'Title & Company'}</p></div>
