        ...
        <div className="space-y-3">
          {volunteering.map((vol, i) => (
            <div key={vol.id} className="rounded-xl border border-border overflow-hidden">
              <button onClick={() => setExpandedId(expandedId === vol.id ? null : vol.id)} className="w-full p-4 flex items-center justify-between hover:bg-muted touch-manipulation active:bg-muted/70 min-h-[72px]">
                <DragHandle />
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={e => { e.stopPropagation(); moveDown(i); }} disabled={i === volunteering.length - 1} className="p-1.5 rounded hover:bg-muted disabled:opacity-30 min-w-[40px] min-h-[40px] flex items-center justify-center touch-manipulation" aria-label="Move down"><ArrowDown className="w-3.5 h-3.5" /></button>
                </div>
