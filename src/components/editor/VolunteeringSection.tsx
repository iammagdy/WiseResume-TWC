...
import { Plus, Trash2, ChevronDown, ChevronUp, Heart, Calendar, Building2, Clock } from 'lucide-react';
          ...
          {volunteering.map((vol, i) => (
            <div key={vol.id} className="rounded-xl border border-border overflow-hidden">
              <button onClick={() => setExpandedId(expandedId === vol.id ? null : vol.id)} className="w-full p-4 flex items-center justify-between hover:bg-muted/50 touch-manipulation active:bg-muted/70 min-h-[72px]">
                <div className="text-left flex-1 min-w-0 pr-3"><p className="font-semibold text-sm truncate">{vol.role || `Volunteer ${i + 1}`}</p><p className="text-sm text-muted-foreground truncate">{vol.organization || 'Organization'}</p></div>
