...
function ChatErrorCard({
  ...
  const tone =
    info.kind === 'credits' || info.kind === 'invalid_key'
      ? { bar: 'bg-amber-500', text: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-500/5 border-amber-500/30' }
      : info.kind === 'rate_limit_client' || info.kind === 'rate_limit_server'
      ? { bar: 'bg-blue-500', text: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-500/5 border-blue-500/30' }
      : { bar: 'bg-red-500', text: 'text-red-600 dark:text-red-400', bg: 'bg-red-500/5 border-red-500/30' };

  return (
    <div className={cn('rounded-xl border p-3 space-y-2.5', tone.bg)}>
      <div className="flex items-start gap-2">
        <AlertCircle className={cn('w-4 h-4 mt-0.5 shrink-0', tone.text)} />
        <div className="min-w-0 flex-1">
          <p className={cn('text-sm font-semibold leading-tight', tone.text)}>{info.title}</p>
