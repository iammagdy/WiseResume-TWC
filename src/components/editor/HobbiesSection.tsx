...
export const HobbiesSection = memo(function HobbiesSection() {
  ...
  const addHobby = () => {
    haptics.light();
    const n: Hobby = { id: uuidv4(), name: '', visible: true };
    updateResume({ hobbies: [n, ...hobbies] });
  };

  const updateHobby = (id: string, u: Partial<Hobby>) => {
    updateResume({
      hobbies: hobbies.map(h => h.id === id ? { ...h, ...u } : h)
    });
  };

  const deleteHobby = (id: string) => {
    haptics.light();
    updateResume({ hobbies: hobbies.filter(h => h.id !== id) });
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-end">
        <Button variant="outline" size="sm" onClick={addHobby} className="gap-2 active:scale-95 transition-transform">
      ...
      {hobbies.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p className="text-sm">{t('editor.hobbies.emptyTip', 'Add your hobbies & interests')}</p>
        </div>
      ) : (
            ...
            <div key={hobby.id} className="rounded-xl border border-border p-4 space-y-3 transition-transform duration-200">
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <Input
                    aria-label={t('editor.sections.hobbies', 'Hobbies')}
                    value={hobby.name}
                    onChange={e => updateHobby(hobby.id, { name: e.target.value })}
                    placeholder={t('editor.hobbies.namePlaceholder', 'Photography, Hiking, etc.')}
                    className="h-12"
                  />
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    onClick={() => updateHobby(hobby.id, { visible: !hobby.visible })}
                    className="p-2 rounded-lg hover:bg-muted active:scale-95 transition-all touch-manipulation min-w-[44px] min-h-[44px] flex items-center justify-center"
                    aria-label={hobby.visible ? t('common.hide', 'Hide') : t('common.show', 'Show')}
                  >
              ...
              <Textarea
                ...
                placeholder={t('editor.hobbies.descriptionPlaceholder', 'Brief description (optional)')}
