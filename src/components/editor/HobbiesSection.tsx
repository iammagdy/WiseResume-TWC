      ...
      {hobbies.length === 0 ? (
        ...
        <div className="space-y-3">
          {hobbies.map((hobby, index) => (
            <div key={hobby.id} className="rounded-xl border border-border p-4 space-y-3">
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={() => moveDown(index)} disabled={index === hobbies.length - 1} className="p-1.5 rounded hover:bg-muted disabled:opacity-30 min-w-[32px] min-h-[32px] flex items-center justify-center active:scale-95" aria-label="Move down"><ArrowDown className="w-3.5 h-3.5" /></button>
                </div>
