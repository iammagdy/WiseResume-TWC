...
export const ElegantTemplate = memo(function ElegantTemplate({ resume }: ElegantTemplateProps) {
  ...
  return (
    <div className="w-[612px] min-h-[792px] bg-white text-gray-800 font-sans text-sm">
      {/* Elegant Header with Soft Background */}
      <header className="px-8 py-8 text-center" style={{ backgroundColor: roseLight }}>
        <h1 className="text-3xl font-light tracking-wide mb-2" style={{ color: roseColor }}>
        ...
        {(contactInfo.linkedin || contactInfo.portfolio) && (
          <div className="flex justify-center gap-4 text-sm mt-2" style={{ color: roseColor }}>
      ...
      <div className="px-8 py-6">
        ...
        {summary && (
          <section className="mb-6" data-section="summary">
            <div className="relative py-4 px-6 rounded-lg border" style={{ borderColor: roseMuted }}>
              <div 
                className="absolute -top-2.5 left-6 px-2 bg-white text-xs uppercase tracking-widest"
        ...
        {experience.length > 0 && (
          <section className="mb-6" data-section="experience">
            <h2 className="text-sm font-medium uppercase tracking-widest mb-4 pb-2 border-b-2" style={{ color: roseColor, borderColor: roseMuted }}>
              Experience
            </h2>
                      ...
                      {exp.achievements.map((achievement, idx) => (
                        <li key={idx} className="text-gray-700 text-sm pl-3 relative">
                          <span 
                            ...
                            style={{ backgroundColor: roseMuted }}
                          />
                          {achievement}
                        </li>
                      ))}
        ...
        {/* Skills as Elegant Pills */}
        {skills.length > 0 && (
          <section className="mb-6" data-section="skills">
            <h2 className="text-sm font-medium uppercase tracking-widest mb-3 pb-2 border-b-2" style={{ color: roseColor, borderColor: roseMuted }}>
              Expertise
            </h2>
            <div className="flex flex-wrap gap-2">
              {skills.map((skill, idx) => (
                <span 
                  ...
                  style={{ borderColor: roseMuted, color: roseColor, backgroundColor: 'white' }}
                >
                  {skill}
                </span>
              ))}
