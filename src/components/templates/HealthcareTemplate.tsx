    ...
    <div className="w-[612px] min-h-[792px] bg-white text-gray-900 font-sans text-sm">
      ...
      <header className="px-8 py-6" style={{ backgroundColor: tealColor }}>
        ...
        {(contactInfo.linkedin || contactInfo.portfolio) && (
          <div className="flex gap-4 text-teal-100 text-sm mt-1">
      ...
      <div className="px-8 py-6">
        ...
        {summary && (
          <section className="mb-5" data-section="summary">
            <h2 className="text-sm font-bold uppercase tracking-wide mb-2 flex items-center gap-2" style={{ color: tealColor }}>
              <span className="w-1 h-4 rounded" style={{ backgroundColor: tealColor }} />
        ...
        {/* Certifications - Important in Healthcare */}
            ...
            <div className="space-y-4">
              {experience.map((exp) => (
                <div key={exp.id} className="border-l-2 pl-3" style={{ borderColor: '#99f6e4' }}>
                  ...
                  {exp.description && (
                    <p data-break-child className="text-gray-600 text-sm mt-1">{exp.description}</p>
                  )}
                  {exp.achievements.length > 0 && (
                    <ul data-break-child className="mt-2 space-y-1">
                      {exp.achievements.map((achievement, idx) => (
                        <li key={idx} className="text-gray-700 text-sm flex items-start gap-2">
                          <span style={{ color: tealColor }}>✓</span>
                          {achievement}
                        </li>
                      ))}
                    </ul>
