    ...
    <div className="p-8 font-sans text-sm leading-relaxed">
          ...
          <div className="space-y-4">
            {resume.experience.map(exp => (
              <div key={exp.id} data-break-avoid>
                <div className="flex justify-between items-start"><div><h3 className="font-bold text-gray-900">{exp.position}</h3><p className="text-indigo-600 text-xs">{exp.company}{exp.account && ` · ${exp.account}`}</p></div><span className="text-xs text-gray-400">{formatDateRangeDisplay(exp.startDate, exp.endDate, exp.current)}</span></div>
      ...
      {resume.education.length > 0 && (
        <section data-section="education" className="mb-6">
          ...
          {resume.education.map(edu => (<div key={edu.id} data-break-avoid className="flex justify-between mb-2"><div><h3 className="font-bold text-gray-900 text-xs">{formatDegreeAndField(edu.degree, edu.field)}</h3><p className="text-gray-600 text-xs">{edu.institution}</p>{edu.description && <p className="text-gray-600 text-xs mt-0.5">{edu.description}</p>}</div><span className="text-xs text-gray-400">{formatDateRangeDisplay(edu.startDate, edu.endDate, edu.endDate === 'Present')}</span></div>))}
        </section>
      )}
