...
export const CreativeTemplate = memo(function CreativeTemplate({ resume }: TemplateProps) {
  const initials = resume.contactInfo.fullName?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) || 'CV';
    ...
    <div className="min-h-full text-xs font-sans bg-white">
      <header className="bg-gradient-to-r from-violet-600 to-purple-700 text-white p-6">
        <div className="flex items-center gap-4 mb-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">{resume.contactInfo.fullName || 'Your Name'}</h1>
            {resume.experience[0]?.position && <p className="text-violet-200 font-medium mt-1">{resume.experience[0].position}</p>}
          </div>
        </div>
        <ContactLinks contact={resume.contactInfo} className="text-violet-100 text-xs" iconSize={3} />
      </header>
      ...
      <div className="p-6 space-y-6">
        ...
        {resume.experience.length > 0 && (
          <section data-section="experience">
            <h2 className="text-sm font-bold text-gray-900 mb-3 flex items-center gap-2"><span className="w-6 h-0.5 bg-violet-500"></span>Experience</h2>
        ...
        {resume.skills.length > 0 && (
          <section data-section="skills">
            <h2 className="text-sm font-bold text-gray-900 mb-3 flex items-center gap-2"><span className="w-6 h-0.5 bg-violet-500"></span>Skills</h2>
            <div className="flex flex-wrap gap-1.5">
              {resume.skills.map((skill, i) => (<span key={i} className="px-2 py-0.5 bg-violet-100 text-violet-700 rounded-full text-xs">{skill}</span>))}
            </div>
          </section>
        )}
        {resume.education.length > 0 && (
          <section data-section="education">
            <h2 className="text-sm font-bold text-gray-900 mb-3 flex items-center gap-2"><span className="w-6 h-0.5 bg-violet-500"></span>Education</h2>
