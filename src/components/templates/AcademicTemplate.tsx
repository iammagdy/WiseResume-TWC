...
export const AcademicTemplate = memo(function AcademicTemplate({ resume }: AcademicTemplateProps) {
  ...
  return (
    <div className="w-[612px] min-h-[792px] bg-white text-gray-900 p-8 font-serif text-sm">
      <header className="text-center mb-6 pb-4 border-b-2" style={{ borderColor: '#1e3a5f' }}>
        <h1 className="text-2xl font-bold mb-2" style={{ color: '#1e3a5f' }}>{contactInfo.fullName}</h1>
        <div className="flex justify-center">
          <ContactLinks contact={contactInfo} className="text-gray-600 text-sm" iconSize={3} />
        </div>
      </header>
      ...
      {skills.length > 0 && (
        <section className="mb-5" data-section="skills">
          <h2 className="text-sm font-bold uppercase tracking-wider mb-2 pb-1 border-b" style={{ color: '#1e3a5f', borderColor: '#1e3a5f' }}>Areas of Expertise</h2>
          <div className="flex flex-wrap gap-2">
            {skills.map((skill, idx) => (<span key={idx} className="px-2 py-0.5 text-sm rounded" style={{ backgroundColor: '#e8eef5', color: '#1e3a5f' }}>{skill}</span>))}
          </div>
        </section>
      )}
      {certifications.length > 0 && (
        <section data-section="certifications">
          <h2 className="text-sm font-bold uppercase tracking-wider mb-2 pb-1 border-b" style={{ color: '#1e3a5f', borderColor: '#1e3a5f' }}>Certifications & Awards</h2>
          <div className="space-y-1">
            {certifications.map((cert) => (
              <div key={cert.id} className="flex justify-between"><span className="text-gray-700"><strong>{cert.name}</strong> – {cert.issuer}</span><span className="text-gray-600">{cert.date}</span></div>
            ))}
          </div>
        </section>
      )}
