...
export const BentoTemplate = memo(function BentoTemplate({ resume }: TemplateProps) {
  return (
    <div className="p-7 font-sans text-sm leading-relaxed bg-gray-50">
      {/* Header card */}
      <header className="mb-4 p-5 rounded-2xl bg-white border border-gray-200 shadow-sm">
      ...
      {(resume.summary || resume.skills.length > 0) && (
        <div className="grid grid-cols-3 gap-3 mb-3">
          {resume.summary && (
            <section data-section="summary" className="col-span-2 p-4 rounded-2xl bg-white border border-gray-200 shadow-sm">
              <h2 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Profile</h2>
              <p data-break-child className="text-gray-600 text-xs leading-relaxed">{resume.summary}</p>
            </section>
          )}
          {resume.skills.length > 0 && (
            <section data-section="skills" className={`${resume.summary ? 'col-span-1' : 'col-span-3'} p-4 rounded-2xl bg-white border border-gray-200 shadow-sm`}>
      ...
      {resume.experience.length > 0 && (
        <section data-section="experience" className="mb-3 p-4 rounded-2xl bg-white border border-gray-200 shadow-sm">
