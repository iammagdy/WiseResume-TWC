  ...
  return (
    <div className="w-[612px] min-h-[792px] bg-white text-gray-900 font-sans text-sm">
      <header className="px-8 py-6 bg-gray-900 text-white">
        ...
        <ContactLinks contact={contactInfo} className="text-gray-300 text-sm" iconSize={3} />
      </header>
      ...
      <div className="px-8 py-6">
        ...
        {experience.length > 0 && (
          <section className="mb-5" data-section="experience">
            <h2 className="text-sm font-bold uppercase tracking-wide mb-3 text-gray-900">Sales Experience</h2>
                  ...
                  {exp.achievements.length > 0 && (
                    <div data-break-child className="grid grid-cols-2 gap-2">
                      {exp.achievements.map((a, idx) => {
                        const { metric, rest } = extractMetric(a);
                        return (
                          <div key={idx} className="p-2 rounded border bg-gray-50">
                            {metric ? (<><p className="text-lg font-bold" style={{ color: greenColor }}>{metric}</p><p className="text-gray-600 text-xs">{rest || a}</p></>) : (<p className="text-gray-700 text-sm">- {a}</p>)}
                          </div>
                        );
                      })}
                    </div>
        ...
        {skills.length > 0 && (
          <section className="mb-5" data-section="skills">
            <h2 className="text-sm font-bold uppercase tracking-wide mb-2 text-gray-900">Core Competencies</h2>
            <div className="flex flex-wrap gap-2">
              {skills.map((skill, idx) => (<span key={idx} className="px-3 py-1 text-sm font-medium rounded-full" style={{ backgroundColor: greenLight, color: greenColor }}>{skill}</span>))}
            </div>
          </section>
        )}
        <div className="grid grid-cols-2 gap-6">
          ...
          {certifications.length > 0 && (
            <section data-section="certifications">
              <h2 className="text-sm font-bold uppercase tracking-wide mb-2 text-gray-900">Certifications</h2>
              <div className="space-y-1">
                {certifications.map((cert) => (<div key={cert.id} data-break-avoid><p className="font-medium text-gray-900 text-sm">{cert.name}</p><p className="text-gray-500 text-xs">{cert.issuer}{cert.date ? `, ${cert.date}` : ''}</p></div>))}
              </div>
            </section>
          )}
        </div>
