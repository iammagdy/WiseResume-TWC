    ...
    <div className="p-8 font-serif text-sm leading-relaxed">
          ...
          <div className="space-y-3">
            {resume.experience.map(exp => (
              <div key={exp.id} data-break-avoid>
                <div className="flex justify-between"><h3 className="font-bold text-gray-900 text-xs">{exp.position} — {exp.company}</h3><span className="text-xs text-gray-500">{formatDateRangeDisplay(exp.startDate, exp.endDate, exp.current)}</span></div>
      ...
      {resume.education.length > 0 && (
        <section data-section="education" className="mb-5">
          ...
          {resume.education.map(edu => (<div key={edu.id} data-break-avoid className="mb-2"><span className="font-bold text-gray-900 text-xs">{formatDegreeAndField(edu.degree, edu.field).replace(" in ", " — ")}</span>, <span className="text-gray-600 text-xs">{edu.institution}</span> <span className="text-gray-500 text-xs">({formatDateRangeDisplay(edu.startDate, edu.endDate, edu.endDate === 'Present')})</span>{edu.description && <p className="text-gray-600 text-xs mt-0.5">{edu.description}</p>}</div>))}
        </section>
      )}
