      ...
      {resume.education.length > 0 && (
        <section data-section="education">
          ...
          {resume.education.map(edu => {
            ...
            return (<div key={edu.id} data-break-avoid className="mb-2"><h3 className="font-bold text-gray-900 text-xs">{formatDegreeAndField(edu.degree, edu.field).replace(" in ", " — ")}</h3><p className="text-gray-600 text-xs">{edu.institution}{eduRange && `, ${eduRange}`}</p>{edu.description && <p className="text-gray-600 text-xs mt-0.5">{edu.description}</p>}</div>);
          })}
        </section>
      )}
