    ...
    <div className="p-10 font-sans text-sm leading-relaxed">
          ...
          <div className="space-y-5">
            {resume.experience.map(exp => (
              <div key={exp.id} data-break-avoid>
                <div className="flex justify-between items-baseline"><h3 className="font-medium text-gray-900">{exp.position}</h3><span className="text-xs text-gray-400">{exp.startDate} – {exp.current ? 'Present' : exp.endDate}</span></div>
      ...
      {resume.education.length > 0 && (
        <section data-section="education" className="mb-8">
          ...
          {resume.education.map(edu => (<div key={edu.id} data-break-avoid className="flex justify-between mb-3"><div><h3 className="font-medium text-gray-900 text-xs">{edu.degree} {edu.field && `in ${edu.field}`}</h3><p className="text-gray-500 text-xs">{edu.institution}</p></div><span className="text-xs text-gray-400">{edu.endDate}</span></div>))}
        </section>
      )}
