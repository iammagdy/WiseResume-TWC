...
// via the x-editor-ai-action: recruiter-sim dispatch path.
//
// The full original source is preserved in git history (pre-Task-#41 commits).
// Rollback: `supabase functions deploy recruiter-simulation` from a pre-retirement tag,
...
serve(async (req) => {
  ...
  return new Response(
    JSON.stringify({ error: 'recruiter-simulation has been retired. Use editor-ai with x-editor-ai-action: recruiter-sim.' }),
