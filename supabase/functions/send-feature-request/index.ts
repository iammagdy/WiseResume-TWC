...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { feature_title, feature_description, user_id, user_email, user_agent, app_version, route } = body;
...
<tr><td style="background-color:#1a1a2e;padding:24px 40px;border-radius:0 0 12px 12px" align="center">
  <p style="margin:0;font-size:12px;color:rgba(255,255,255,0.5)">WiseResume Feature Request · Reply to reach the user</p>
</td></tr>
        ...
        const emailRes = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            subject: `[Feature] ${feature_title.slice(0, 80)}`,
            html: emailHtml,
          }),
        });
