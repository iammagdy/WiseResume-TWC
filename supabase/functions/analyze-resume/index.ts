...
// via the x-editor-ai-action: analyze dispatch path.
//
// The full original source is preserved in git history (pre-Task-#41 commits).
// Rollback: `supabase functions deploy analyze-resume` from a pre-retirement tag,
...
Deno.serve(async (req) => {
  ...
  return new Response(
    JSON.stringify({ error: 'analyze-resume has been retired. Use editor-ai with x-editor-ai-action: analyze.' }),
