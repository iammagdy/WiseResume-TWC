...
// via the x-editor-ai-action: suggest-template dispatch path.
//
// The full original source is preserved in git history (pre-Task-#41 commits).
// Rollback: `supabase functions deploy suggest-template` from a pre-retirement tag,
...
Deno.serve(async (req) => {
  ...
  return new Response(
    JSON.stringify({ error: 'suggest-template has been retired. Use editor-ai with x-editor-ai-action: suggest-template.' }),
