...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (rateLimitKey === "unknown" && !resolvedUserId) {
      console.warn("[send-contact-inquiry] Could not determine caller identity for rate limiting");
    }

    const { allowed } = await checkRateLimit(rateLimitKey, { actionType: 'contact_inquiry', maxRequests: 5, windowSeconds: 300 });
    if (!allowed) {
      return new Response(
        JSON.stringify({ error: "Too many requests. Please wait 5 minutes before sending another inquiry." }),
        { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json", "Retry-After": "300" } }
      );
    }
    await recordUsage(rateLimitKey, 'contact_inquiry');
...
<tr><td style="background-color:#1a1a2e;padding:32px 40px;border-radius:12px 12px 0 0" align="center">
  ...
  <h1 style="margin:0;font-size:20px;font-weight:700;color:#ffffff">📩 Contact Inquiry${department ? ` — ${escapeHtml(department)}` : ""}</h1>
...
<tr><td style="padding:24px 40px 0;background-color:#ffffff">
  ...
  <div style="background:#eef2ff;border:1px solid #c7d2fe;border-radius:8px;padding:14px 16px">
    <p style="margin:0;font-size:15px;color:#1a1a2e;font-weight:600;line-height:1.5">${escapeHtml(subject.slice(0, 200))}</p>
  </div>
</td></tr>
...
<tr><td style="padding:20px 40px;background-color:#ffffff">
  ...
  <div style="background:#f8f9fa;border:1px solid #e5e7eb;border-radius:8px;padding:14px 16px">
    <p style="margin:0;font-size:14px;color:#374151;line-height:1.6;white-space:pre-wrap">${escapeHtml(message.slice(0, 2000))}</p>
  </div>
</td></tr>
