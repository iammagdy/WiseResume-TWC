...
-- Public read policy: only exposes rows where portfolio is enabled
-- (column-level privacy is enforced by only selecting last_active_at + username in queries)
CREATE POLICY "Public can view last_active_at for enabled portfolios"
ON public.profiles FOR SELECT
USING (portfolio_enabled = true);

-- Update get_public_portfolio RPC to expose lastActiveAt
