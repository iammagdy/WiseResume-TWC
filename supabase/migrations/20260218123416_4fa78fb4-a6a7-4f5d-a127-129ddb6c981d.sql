...
-- Add new portfolio customization columns to profiles table (all nullable/backward-compatible)
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS portfolio_layout text DEFAULT 'single',
  ADD COLUMN IF NOT EXISTS portfolio_accent_color text DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS portfolio_font text DEFAULT 'inter',
  ADD COLUMN IF NOT EXISTS portfolio_style text DEFAULT 'minimal',
  ADD COLUMN IF NOT EXISTS open_to_work boolean DEFAULT false,
  ADD COLUMN IF NOT EXISTS availability_headline text DEFAULT NULL;

-- Update the get_public_portfolio RPC to include the new fields
