ALTER TABLE public.portfolio_visits
  ADD COLUMN IF NOT EXISTS device text CHECK (device IN ('mobile', 'desktop', 'tablet'));
...
BEGIN
  IF NOT EXISTS (
    ...
    RETURN;
  END IF;
  INSERT INTO public.portfolio_visits (username, country, city, referrer, short_link_id, sections_viewed, time_spent_seconds, device)
  VALUES (lower(p_username), p_country, p_city, p_referrer, p_short_link_id, p_sections_viewed, p_time_spent_seconds, p_device);
END;
$$;
