...
/**
 * Idempotently provisions the two server-only RevenueCat collections.
 * This script is repository-controlled and intentionally unexecuted in this task.
...
const COLLECTION_SPECS = Object.freeze([
  {
    id: 'revenuecat_subscription_state',
    name: 'RevenueCat Subscription State',
    attributes: [
  ...
  {
    id: 'revenuecat_event_ledger',
    name: 'RevenueCat Event Ledger',
    attributes: [
      { key: 'event_id', type: 'string', size: 128, required: true, array: false },
      { key: 'event_type', type: 'string', size: 32, required: true, array: false },
...
async function run() {
  ...
  console.log('RevenueCat provider-state schemas are ready (server-only collections).');
}

if (require.main === module) {
  run().catch(err => { console.error(`RevenueCat schema setup failed: ${err.message}`); process.exit(1); });
}
