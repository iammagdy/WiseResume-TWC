...
- `src/hooks/useResumeShares.ts` (follow-up fix) — removed `'password'` from `Query.select` entirely; public lookup now requests only `$id`, `$createdAt`, `resume_id`, `user_id`, `token`, `is_active`, `expires_at`, `view_count`, `has_password`. `docToPublicShare` no longer references `doc.password`. Password gate relies solely on `has_password` boolean and server-side `edgeFunctions.invoke('verify-share-password')`.
- `src/hooks/useAgenticChat.ts` (second review fix) — added `Query.equal('user_id', user.id)` to the session bootstrap `listDocuments` call (and to `loadSession` initial query). Without this, the query fetched the globally most-recent session regardless of owner, which is a broken-access-control risk.
