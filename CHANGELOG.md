...
- **A3**: New `POST /api/admin/ai-provider/audit-model-switch` and `POST /api/admin/ai-provider/audit-test` endpoints plus `writeAdminAudit()` helper writing to the new `admin_audit_log` Drizzle table. Model switches are recorded with `action='model-switch'`; OpenRouter / Groq / Ollama tests are recorded with `action='provider-test'` and a structured payload `{ provider, model, ok, latencyMs, error }`. Gemini tests are audited server-side inside `/gemini-test` itself (no duplicate client-side write).
