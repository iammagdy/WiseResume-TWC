...
## Decision #8: WiseHire Desktop-First Exception (Phase 1 & 2) — RETIRED 2026-04-17
...
* **Decision (original)**: WiseHire Phase 1 and 2 were explicitly **desktop-first**. Mobile responsive support was deferred to Phase 3. This was a documented, time-limited exception — not a permanent override of the mobile-first rule.
* **Status (2026-04-17)**: **Retired.** Phase 1 is shipped end-to-end and Phases 2/3 are partially shipped, so the exception window has closed. All `/wisehire/*` HR routes have been brought up to the same `xs`/375px mobile-baseline as WiseResume (no horizontal scroll, no clipped controls). The platform-wide mobile-first rule from `PRODUCT.md` now applies to WiseHire without exception. Note: the public job board (`/jobs/*`) referenced in the original Phase 3 mobile scope was never shipped (see AUDIT-2026-04 §A5 and the retracted ADR #11), so it is out of scope for this retirement.
* **Consequences**: All future WiseHire work must meet the mobile-baseline quality bar. Any new HR-facing route must render cleanly at 375px before merge.
