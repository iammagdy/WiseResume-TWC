...
- **ARCHITECTURE.md**: Comprehensive rewrite. Full tech stack table. Full Kinde → Supabase Token Bridge documentation. All 29 current database tables documented with purpose. All 5 Supabase Storage buckets documented (including planned `candidate-resumes` for WiseHire). All 78 edge functions listed and categorized. Full AI system: credit limits, atomic deduction, BYOK (AES-GCM-256), multi-layer rate limiting, fail-closed behavior, WiseHire AI rate limits per tier. WiseHire routing table. Full security and privacy rules. Full modification rules.
