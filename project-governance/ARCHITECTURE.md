...
| Edge functions | Supabase Edge Functions (Deno runtime) — 78 functions |
...
## 5. Database Tables (Current Schema as of 2026-04-15)

All tables use RLS. The authoritative source of truth is `src/integrations/supabase/types.ts` (auto-generated) and the SQL migrations in `supabase/migrations/`.
...
| `admin_audit_log` | Admin-specific audit trail for admin panel actions |
| `admin_user_notes` | Admin sticky notes attached to user accounts |
| `app_settings` | Platform-wide configuration key-value store |
| `notifications` | In-app alerts for users |
| `signup_otps` | OTP codes for email-based sign-up verification |
...
| `resume_snapshots` | Point-in-time captures of full resume state |
...
| `portfolio_history` | Historical snapshots of portfolio state |
...
| `interview_answers` | Individual answers within an interview session |
| `interview_report_tokens` | Shareable token-gated links for interview reports |
