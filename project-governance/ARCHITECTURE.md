...
The authoritative source of table truth is `src/integrations/supabase/types.ts` (auto-generated by Supabase CLI). **Always read that file directly before writing any new table-dependent code.** The inventory below reflects the 39 confirmed tables as of 2026-04-15. Additional tables may exist in SQL migrations that have not yet been promoted to the generated types — treat those as unverified until confirmed in `types.ts`.
